...
// time.
//
#ifndef DELTA_ACCOUNT_STATISTICS_SPACE_ID
#define DELTA_ACCOUNT_STATISTICS_SPACE_ID 10
#endif

#ifndef DELTA_ACCOUNT_STATISTICS_PLUGIN_NAME
#define DELTA_ACCOUNT_STATISTICS_PLUGIN_NAME "account_stats"
#endif
...
{
   account_stats_bucket_object_type    = ( DELTA_ACCOUNT_STATISTICS_SPACE_ID << 8 ),
   account_activity_bucket_object_type = ( DELTA_ACCOUNT_STATISTICS_SPACE_ID << 8 ) + 1
};
...
struct account_stats_bucket_object : public object< account_stats_bucket_object_type, account_stats_bucket_object >
{
   ...
   share_type           author_rewards_total_delta_value = 0;     ///< DELTA Value of author rewards
   ...
   share_type           curation_rewards_delta_value = 0;         ///< DELTA Value of curation rewards
   ...
   share_type           liquidity_rewards = 0;                    ///< Amount of DELTA paid as liquidity rewards
   ...
   share_type           delta_sent = 0;                           ///< DELTA sent from this account
   share_type           delta_received = 0;                       ///< DELTA received by this account
   ...
   share_type           delta_vested = 0;                         ///< DELTA vested by the account
   ...
   share_type           delta_received_from_withdrawls = 0;       ///< DELTA received from this account's vesting withdrawals
   share_type           delta_received_from_routes = 0;           ///< DELTA received from another account's vesting withdrawals
   ...
   share_type           delta_converted = 0;                      ///< Amount of DELTA that was converted
   ...
   share_type           limit_order_delta_paid = 0;               ///< DELTA paid by limit orders
   share_type           limit_order_delta_received = 0;           ///< DELTA received from limit orders
...
class account_statistics_plugin : public delta::app::plugin
{
   public:
      account_statistics_plugin( application* app );
      virtual ~account_statistics_plugin();

      virtual std::string plugin_name()const override { return DELTA_ACCOUNT_STATISTICS_PLUGIN_NAME; }
...
FC_REFLECT( delta::account_statistics::account_stats_bucket_object,
   (id)
   (open)
   (seconds)
   (name)
   ...
   (vests_withdrawn)
   (delta_received_from_withdrawls)
   (delta_received_from_routes)
   ...
   (limit_order_delta_paid)
   (limit_order_delta_received)
   (limit_order_dbd_paid)
   (limit_order_dbd_received)
   (total_pow)
   (estimated_hashpower)
)
//SET_INDEX_TYPE( delta::account_statistics::account_stats_bucket_object,)

FC_REFLECT(
   delta::account_statistics::account_activity_bucket_object,
   (id)
   (open)
   (seconds)
