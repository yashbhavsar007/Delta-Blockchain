...
#include <delta/protocol/sign_state.hpp>

#include <delta/chain/account_object.hpp>
#include <delta/chain/database.hpp>

#include <delta/plugins/auth_util/auth_util_api.hpp>
#include <delta/plugins/auth_util/auth_util_plugin.hpp>
...
{
   public:
      auth_util_api_impl( delta::app::application& _app );
      void check_authority_signature( const check_authority_signature_params& args, check_authority_signature_result& result );

      std::shared_ptr< delta::plugin::auth_util::auth_util_plugin > get_plugin();

      delta::app::application& app;
};

auth_util_api_impl::auth_util_api_impl( delta::app::application& _app ) : app( _app )
{}

std::shared_ptr< delta::plugin::auth_util::auth_util_plugin > auth_util_api_impl::get_plugin()
{
...
auth_util_api::auth_util_api( const delta::app::api_context& ctx )
{
