...
{
   public:
      auth_util_api( const delta::app::api_context& ctx );
...
FC_REFLECT( delta::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( delta::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( delta::plugin::auth_util::auth_util_api,
