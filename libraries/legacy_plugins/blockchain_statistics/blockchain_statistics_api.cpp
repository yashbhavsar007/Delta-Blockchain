...
namespace detail
{
   ...
   {
      public:
         blockchain_statistics_api_impl( delta::app::application& app )
            :_app( app ) {}
   ...
   statistics blockchain_statistics_api_impl::get_stats_for_interval( fc::time_point_sec start, fc::time_point_sec end )const
   {
      ...
      const auto& sizes = _app.get_plugin< blockchain_statistics_plugin >( DELTA_BLOCKCHAIN_STATISTICS_PLUGIN_NAME )->get_tracked_buckets();
...
blockchain_statistics_api::blockchain_statistics_api( const delta::app::api_context& ctx )
{
...
statistics& statistics::operator +=( const bucket_object& b )
{
   ...
   this->delta_transferred                      += b.delta_transferred;
   ...
   this->delta_vested                           += b.delta_vested;
   ...
   this->delta_converted                        += b.delta_converted;
