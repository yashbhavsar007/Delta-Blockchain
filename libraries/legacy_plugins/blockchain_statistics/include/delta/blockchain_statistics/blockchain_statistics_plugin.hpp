...
// time.
//
#ifndef DELTA_BLOCKCHAIN_STATISTICS_SPACE_ID
#define DELTA_BLOCKCHAIN_STATISTICS_SPACE_ID 9
#endif

#ifndef DELTA_BLOCKCHAIN_STATISTICS_PLUGIN_NAME
#define DELTA_BLOCKCHAIN_STATISTICS_PLUGIN_NAME "chain_stats"
#endif

namespace delta { namespace blockchain_statistics {
...
{
   bucket_object_type = ( DELTA_BLOCKCHAIN_STATISTICS_SPACE_ID << 8 )
};
...
class blockchain_statistics_plugin : public delta::app::plugin
{
   public:
      blockchain_statistics_plugin( application* app );
      virtual ~blockchain_statistics_plugin();

      virtual std::string plugin_name()const override { return DELTA_BLOCKCHAIN_STATISTICS_PLUGIN_NAME; }
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   share_type           delta_transferred = 0;                       ///< DELTA transferred from account to account
   ...
   share_type           liquidity_rewards_paid = 0;                  ///< Ammount of DELTA paid to market makers
   uint32_t             transfers_to_vesting = 0;                    ///< Transfers of DELTA into VESTS
   share_type           delta_vested = 0;                            ///< Ammount of DELTA vested
   ...
   share_type           vests_withdrawn = 0;                         ///< Ammount of VESTS withdrawn to DELTA
...
} } // delta::blockchain_statistics

FC_REFLECT( delta::blockchain_statistics::bucket_object,
   (id)
   (open)
   (seconds)
   (blocks)
...
CHAINBASE_SET_INDEX_TYPE( delta::blockchain_statistics::bucket_object, delta::blockchain_statistics::bucket_index )
