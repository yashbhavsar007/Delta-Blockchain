...
#include <delta/plugins/condenser_api/condenser_api.hpp>
...
using namespace delta::utilities;
...
{
   public:
      wallet_api( const wallet_data& initial_data, const delta::protocol::chain_id_type& _delta_chain_id, fc::api< remote_node_api > rapi );
      virtual ~wallet_api();
       ...
       *  'info' wallet command.
       *
       *  These accounts are created with combination of DELTA and delegated TX
       *
       *  @param creator The account creating the new account
       *  @param delta_fee The amount of the fee to be paid with DELTA
      ...
      /**
       * Transfer funds from one account to another. DELTA and DBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       * @param amount The funds being transferred. i.e. "100.000 DELTA"
      ...
      /**
       * Transfer funds from one account to another using escrow. DELTA and DBD can be transferred.
       *
       * @param from The account the funds are coming from
       * @param to The account the funds are going to
       ...
       * @param delta_amount The amount of DELTA to transfer
       ...
       * @param delta_amount The amount of DELTA that will be released
       * @param broadcast true if you wish to broadcast the transaction
       */
      ...
      /**
       * Transfer DELTA into a vesting fund represented by vesting shares (VESTS). VESTS are required to vesting
       * for a minimum of one coin year and can be withdrawn once a week over a two year withdraw period.
       * VESTS are protected against dilution up until 90% of DELTA is vesting.
       *
       * @param from The account the DELTA is coming from
       * @param to The account getting the VESTS
       * @param amount The amount of DELTA to vest i.e. "100.00 DELTA"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *    withdrawn and deposited back as DELTA. i.e. "10.000000 VESTS"
       * @param broadcast true if you wish to broadcast the transaction
       */
       ...
       *
       * @param from The account the VESTS are withdrawn from.
       * @param to   The account receiving either VESTS or DELTA.
      ...
      /**
       *  This method will convert DBD to DELTA at the current_median_history price one
      ...
      /**
       * A witness can public a price feed for the DELTA:DBD market. The median price feed is used
       * to process conversion requests from DBD to DELTA.
       *
       ...
       *                       operations defined in `delta/chain/operations.hpp`
      ...
      /**
       * Gets the current order book for DELTA:DBD
       *
       ...
       *  @param amount_to_sell The amount of either DBD or DELTA you wish to sell
      ...
      /**
       * Vote on a comment to be paid DELTA
       *
...
FC_REFLECT( delta::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( delta::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( delta::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT_ENUM( delta::wallet::authority_type, (owner)(active)(posting) )

FC_API( delta::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
...
FC_REFLECT( delta::wallet::memo_data, (from)(to)(nonce)(check)(encrypted) )
