...
#define DELTA_CHAIN_PLUGIN_NAME "chain"
...
public:
   ...
   static const std::string& name() { static std::string name = DELTA_CHAIN_PLUGIN_NAME; return name; }
   ...
   bool accept_block( const delta::chain::signed_block& block, bool currently_syncing, uint32_t skip );
   void accept_transaction( const delta::chain::signed_transaction& trx );
   delta::chain::signed_block generate_block(
   ...
   bool block_is_on_preferred_chain( const delta::chain::block_id_type& block_id );

   void check_time_in_block( const delta::chain::signed_block& block );
