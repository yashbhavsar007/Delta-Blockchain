...
#include <delta/plugins/stats_export/stats_export_plugin.hpp>
...
using delta::chain::block_notification;
using delta::chain::database;
using delta::chain::dynamic_global_property_object;
...
using delta::plugins::chain::chain_plugin;
...
FC_REFLECT( delta::plugins::stats_export::detail::api_stats_transaction_data_object, (user)(size) )
FC_REFLECT( delta::plugins::stats_export::detail::api_stats_export_data_object, (global_properties)(transaction_stats)(free_memory) )

namespace delta { namespace plugins { namespace stats_export { namespace detail {
...
{
   std::shared_ptr< api_stats_export_data_object > stats = _export_plugin.find_export_data< api_stats_export_data_object >( DELTA_STATS_EXPORT_PLUGIN_NAME );
   if( !stats )
      return;
...
void stats_export_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   try
   {
      ...
      my->_export_plugin.register_export_data_factory( DELTA_STATS_EXPORT_PLUGIN_NAME,
