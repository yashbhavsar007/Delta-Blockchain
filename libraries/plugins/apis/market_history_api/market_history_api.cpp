...
{
   public:
      market_history_api_impl() : _db( appbase::app().get_plugin< delta::plugins::chain::chain_plugin >().db() ) {}
...
DEFINE_API_IMPL( market_history_api_impl, get_ticker )
{
   ...
   {
      auto open = ASSET_TO_REAL( asset( itr->non_delta.open, DBD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->delta.open, DELTA_SYMBOL ) );
      itr = bucket_idx.lower_bound( boost::make_tuple( 0, _db.head_block_time() ) );
      result.latest = ASSET_TO_REAL( asset( itr->non_delta.close, DBD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->delta.close, DELTA_SYMBOL ) );
   ...
   result.delta_volume = volume.delta_volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_volume )
{
   ...
   do
   {
      result.delta_volume.amount += itr->delta.volume;
      result.dbd_volume.amount += itr->non_delta.volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_order_book )
{
   ...
   auto itr = order_idx.lower_bound( price::max( DBD_SYMBOL, DELTA_SYMBOL ) );
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == DBD_SYMBOL && result.bids.size() < args.limit )
   {
      ...
      cur.delta = ( asset( itr->for_sale, DBD_SYMBOL ) * itr->sell_price ).amount;
   ...
   itr = order_idx.lower_bound( price::max( DELTA_SYMBOL, DBD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == DELTA_SYMBOL && result.asks.size() < args.limit )
   {
      order cur;
      cur.order_price = itr->sell_price;
      cur.real_price = ASSET_TO_REAL( itr->sell_price.quote ) / ASSET_TO_REAL( itr->sell_price.base );
      cur.delta = itr->for_sale;
      cur.dbd = ( asset( itr->for_sale, DELTA_SYMBOL ) * itr->sell_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( market_history_api_impl, get_market_history_buckets )
{
   ...
   result.bucket_sizes = appbase::app().get_plugin< delta::plugins::market_history::market_history_plugin >().get_tracked_buckets();
   return result;
}
...
{
   JSON_RPC_REGISTER_API( DELTA_MARKET_HISTORY_API_PLUGIN_NAME );
}
