...
#include <delta/plugins/account_history_api/account_history_api_plugin.hpp>
#include <delta/plugins/account_by_key_api/account_by_key_api_plugin.hpp>
#include <delta/plugins/network_broadcast_api/network_broadcast_api_plugin.hpp>
#include <delta/plugins/tags_api/tags_api_plugin.hpp>
#include <delta/plugins/follow_api/follow_api_plugin.hpp>
#include <delta/plugins/reputation_api/reputation_api_plugin.hpp>
#include <delta/plugins/market_history_api/market_history_api_plugin.hpp>
...
namespace detail
{
   ...
   class condenser_api_impl
   {
      public:
         condenser_api_impl() :
            _chain( appbase::app().get_plugin< delta::plugins::chain::chain_plugin >() ),
            _db( _chain.db() )
         {
            _on_post_apply_block_conn = _db.add_post_apply_block_handler(
               [&]( const block_notification& note ){ on_post_apply_block( note.block ); },
               appbase::app().get_plugin< delta::plugins::condenser_api::condenser_api_plugin >(),
               0 );
         }
         ...
         delta::plugins::chain::chain_plugin&                              _chain;
   ...
   DEFINE_API_IMPL( condenser_api_impl, get_version )
   {
      ...
      (
         fc::string( DELTA_BLOCKCHAIN_VERSION ),
         fc::string( delta::utilities::git_revision_sha ),
   ...
   {
      FC_ASSERT( false, "condenser_api::get_account_references --- Needs to be refactored for Delta." );
   }
   ...
   void condenser_api_impl::set_pending_payout( discussion& d )
   {
      ...
      if( _db.has_hardfork( DELTA_HARDFORK_0_17__774 ) )
         ...
         pot = props.total_reward_fund_delta;
      ...
      if( d.parent_author != DELTA_ROOT_POST_PARENT )
...
{
   if( a.voting_manabar.last_update_time < DELTA_HARDFORK_0_20_TIME )
   ...
   // vp_t2 = DELTA_100_PERCENT * current_mana / vests
   // vp_t1 = vp_t2 - DELTA_100_PERCENT * (t2 - t1) / DELTA_VOTING_MANA_REGENERATION_SECONDS
   //

   uint32_t t1 = a.last_vote_time.sec_since_epoch();
   uint32_t t2 = a.voting_manabar.last_update_time;
   uint64_t dt = (t2 > t1) ? (t2 - t1) : 0;
   uint64_t vp_dt = DELTA_100_PERCENT * dt / DELTA_VOTING_MANA_REGENERATION_SECONDS;

   uint128_t vp_t2 = DELTA_100_PERCENT;
   vp_t2 *= a.voting_manabar.current_mana;
   vp_t2 /= vests;

   uint64_t vp_t2u = vp_t2.to_uint64();
   if( vp_t2u >= DELTA_100_PERCENT )
   {
      wlog( "Truncated vp_t2u to DELTA_100_PERCENT for account ${a}", ("a", a.name) );
      vp_t2u = DELTA_100_PERCENT;
   }
...
{
   JSON_RPC_REGISTER_API( DELTA_CONDENSER_API_PLUGIN_NAME );
}
