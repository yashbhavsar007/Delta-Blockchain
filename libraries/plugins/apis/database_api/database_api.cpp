...
#include <delta/protocol/transaction_util.hpp>
...
{
   JSON_RPC_REGISTER_API( DELTA_DATABASE_API_PLUGIN_NAME );
}
...
{
   return delta::protocol::get_config();
}
...
DEFINE_API_IMPL( database_api_impl, get_order_book )
{
   ...
   auto max_sell = price::max( DBD_SYMBOL, DELTA_SYMBOL );
   auto max_buy = price::max( DELTA_SYMBOL, DBD_SYMBOL );
   ...
   while( sell_itr != end && sell_itr->sell_price.base.symbol == DBD_SYMBOL && result.bids.size() < args.limit )
   {
      ...
      cur.delta = ( asset( itr->for_sale, DBD_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
      result.bids.push_back( cur );
      ++sell_itr;
   }
   while( buy_itr != end && buy_itr->sell_price.base.symbol == DELTA_SYMBOL && result.asks.size() < args.limit )
   {
      auto itr = buy_itr;
      order cur;
      cur.order_price = itr->sell_price;
      ...
      cur.dbd     = ( asset( itr->for_sale, DELTA_SYMBOL ) * cur.order_price ).amount;
      cur.created = itr->created;
...
DEFINE_API_IMPL( database_api_impl, get_required_signatures )
{
   ...
   result.keys = args.trx.get_required_signatures( _db.get_chain_id(),
                                                   ...
                                                   _db.has_hardfork( DELTA_HARDFORK_0_20__1944 ) ? fc::ecc::canonical_signature_type::bip_0062 : fc::ecc::canonical_signature_type::fc_canonical );
...
DEFINE_API_IMPL( database_api_impl, verify_signatures )
{
   ...
   try
   {
      delta::protocol::verify_authority< verify_signatures_args >(
         { args },
         sig_keys,
