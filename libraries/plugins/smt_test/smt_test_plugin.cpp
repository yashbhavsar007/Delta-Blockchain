#include <delta/plugins/smt_test/smt_test_plugin.hpp>
#include <delta/plugins/smt_test/smt_test_objects.hpp>
...
void test_alpha()
{
   ...
   uint64_t max_supply = DELTA_MAX_SHARE_SUPPLY / 6000;

   // set delta unit, total is 100 DELTA-satoshis = 0.1 DELTA
   gpolicy.pre_soft_cap_unit.delta_unit.emplace( "founder-a",   7 );
   gpolicy.pre_soft_cap_unit.delta_unit.emplace( "founder-b",  23 );
   gpolicy.pre_soft_cap_unit.delta_unit.emplace( "founder-c",  70 );
   ...
   gpolicy.post_soft_cap_unit.delta_unit.clear();
   gpolicy.post_soft_cap_unit.token_unit.clear();

   gpolicy.min_delta_units_commitment.fillin_nonhidden_value( 1 );
   gpolicy.hard_cap_delta_units_commitment.fillin_nonhidden_value( max_supply );

   gpolicy.soft_cap_percent = DELTA_100_PERCENT;

   // .0006 ALPHA / 0.1 DELTA -> 1000 token-units / delta-unit
...
void test_beta()
{
   ...
   // set delta unit, total is 100 DELTA-satoshis = 0.1 DELTA
   gpolicy.pre_soft_cap_unit.delta_unit.emplace( "fred"  , 3 );
   gpolicy.pre_soft_cap_unit.delta_unit.emplace( "george", 2 );
   ...
   gpolicy.min_delta_units_commitment.fillin_nonhidden_value( 5000000 );
   gpolicy.hard_cap_delta_units_commitment.fillin_nonhidden_value( 30000000 );
...
void test_delta()
{
   ...
   // set delta unit, total is 1 DELTA-satoshi = 0.001 DELTA
   gpolicy.pre_soft_cap_unit.delta_unit.emplace( "founder", 1 );
   ...
   gpolicy.min_delta_units_commitment.fillin_nonhidden_value(      10000000 );
   gpolicy.hard_cap_delta_units_commitment.fillin_nonhidden_value( 10000000 );

   gpolicy.soft_cap_percent = DELTA_100_PERCENT;

   // .001 DELTA / .100000 DELTA -> 100 DELTA / DELTA
   gpolicy.min_unit_ratio = 1000;
   gpolicy.max_unit_ratio = 1000;
