...
using delta::protocol::fill_order_operation;
      ...
      for( const auto& bucket : _tracked_buckets )
      {
         ...
         if( itr == bucket_idx.end() )
         {
            _db.create< bucket_object >( [&]( bucket_object& b )
            {
               ...
               b.delta.fill( ( op.open_pays.symbol == DELTA_SYMBOL ) ? op.open_pays.amount : op.current_pays.amount );
#ifdef DELTA_ENABLE_SMT
                  b.symbol = ( op.open_pays.symbol == DELTA_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
#endif
                  b.non_delta.fill( ( op.open_pays.symbol == DELTA_SYMBOL ) ? op.current_pays.amount : op.open_pays.amount );
            });
         }
         else
         {
            _db.modify( *itr, [&]( bucket_object& b )
            {
               ...
               b.symbol = ( op.open_pays.symbol == DELTA_SYMBOL ) ? op.current_pays.symbol : op.open_pays.symbol;
#endif
               if( op.open_pays.symbol == DELTA_SYMBOL )
               {
                  b.delta.volume += op.open_pays.amount;
                  b.delta.close = op.open_pays.amount;

                  b.non_delta.volume += op.current_pays.amount;
                  b.non_delta.close = op.current_pays.amount;

                  if( b.high() < price( op.current_pays, op.open_pays ) )
                  {
                     b.delta.high = op.open_pays.amount;

                     b.non_delta.high = op.current_pays.amount;
                  }

                  if( b.low() > price( op.current_pays, op.open_pays ) )
                  {
                     b.delta.low = op.open_pays.amount;

                     b.non_delta.low = op.current_pays.amount;
                  }
               }
               else
               {
                  b.delta.volume += op.current_pays.amount;
                  b.delta.close = op.current_pays.amount;

                  b.non_delta.volume += op.open_pays.amount;
                  b.non_delta.close = op.open_pays.amount;

                  if( b.high() < price( op.open_pays, op.current_pays ) )
                  {
                     b.delta.high = op.current_pays.amount;

                     b.non_delta.high = op.open_pays.amount;
                  }

                  if( b.low() > price( op.open_pays, op.current_pays ) )
                  {
                     b.delta.low = op.current_pays.amount;

                     b.non_delta.low = op.open_pays.amount;
                  }
               }
            });
