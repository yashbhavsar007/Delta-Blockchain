...
#include <delta/chain/util/impacted.hpp>

#include <delta/utilities/key_conversion.hpp>
#include <delta/utilities/plugin_utilities.hpp>
...
namespace delta { namespace plugins { namespace witness {
...
namespace detail {
   ...
   public:
      witness_plugin_impl( boost::asio::io_service& io ) :
         _timer(io),
         _chain_plugin( appbase::app().get_plugin< delta::plugins::chain::chain_plugin >() ),
      ...
      uint32_t _required_witness_participation  = 33 * DELTA_1_PERCENT;
      uint32_t _production_skip_flags           = chain::database::skip_nothing;

      std::map< delta::protocol::public_key_type, fc::ecc::private_key > _private_keys;
      std::set< delta::protocol::account_name_type >                     _witnesses;
      boost::asio::deadline_timer                                        _timer;

      std::set< delta::protocol::account_name_type >                     _dupe_customs;
   ...
   struct comment_options_extension_visitor
   {
      ...
      {
         DELTA_ASSERT( cpb.beneficiaries.size() <= 8,
            plugin_exception,
   ...
   void check_memo( const string& memo, const chain::account_object& account, const account_authority_object& auth )
   {
      ...
      {
         for( auto& key : keys )
            DELTA_ASSERT( key_weight_pair.first != key,  plugin_exception,
      ...
      for( auto& key : keys )
         DELTA_ASSERT( memo_key != key,  plugin_exception,
   ...
   struct operation_visitor
   {
      ...
      void operator()( const comment_operation& o )const
      {
         if( o.parent_author != DELTA_ROOT_POST_PARENT )
         {
            ...
            if( parent != nullptr )
            DELTA_ASSERT( parent->depth < DELTA_SOFT_MAX_COMMENT_DEPTH,
               plugin_exception,
               "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",DELTA_SOFT_MAX_COMMENT_DEPTH) );
         }
      }
      ...
      switch( note.op.which() )
      {
         ...
         case operation::tag< custom_binary_operation >::value:
         {
            ...
            for( auto& account : impacted )
               if( _db.is_producing() )
                  DELTA_ASSERT( _dupe_customs.insert( account ).second, plugin_exception,
   ...
   {
      if( fc::time_point::now() < fc::time_point(DELTA_GENESIS_TIME) )
      {
         wlog( "waiting until genesis time to produce block: ${t}", ("t",DELTA_GENESIS_TIME) );
   ...
   block_production_condition::block_production_condition_enum witness_plugin_impl::maybe_produce_block(fc::mutable_variant_object& capture)
   {
      ...
      {
         capture("pct", uint32_t(100*uint64_t(prate) / DELTA_1_PERCENT));
      ...
      appbase::app().get_plugin< delta::plugins::p2p::p2p_plugin >().broadcast_block( block );
...
{ try {
   ...
   DELTA_LOAD_VALUE_SET( options, "witness", my->_witnesses, delta::protocol::account_name_type )
   ...
   if( options.count("private-key") )
   {
      ...
      {
         fc::optional<fc::ecc::private_key> private_key = delta::utilities::wif_to_key(wif_key);
   ...
   {
      my->_required_witness_participation = DELTA_1_PERCENT * options.at( "required-participation" ).as< uint32_t >();
   }
...
{ try {
   ...
   chain::database& d = appbase::app().get_plugin< delta::plugins::chain::chain_plugin >().db();

   if( !my->_witnesses.empty() )
   {
      ilog( "Launching block production for ${n} witnesses.", ("n", my->_witnesses.size()) );
      appbase::app().get_plugin< delta::plugins::p2p::p2p_plugin >().set_block_production( true );
