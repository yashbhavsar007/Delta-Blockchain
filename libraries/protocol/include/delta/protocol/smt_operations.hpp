...
#define SMT_MIN_HARD_CAP_DELTA_UNITS   10000
#define SMT_MIN_SATURATION_DELTA_UNITS 1000
#define SMT_MIN_SOFT_CAP_DELTA_UNITS   1000
...
{
   flat_map< account_name_type, uint16_t >        delta_unit;
   flat_map< account_name_type, uint16_t >        token_unit;

   uint32_t delta_unit_sum()const;
...
struct smt_capped_generation_policy
{
   ...
   smt_cap_commitment  min_delta_units_commitment;
   smt_cap_commitment  hard_cap_delta_units_commitment;
...
struct smt_setup_operation : public smt_base_operation
{
   ...
   int64_t                 max_supply = DELTA_MAX_SHARE_SUPPLY;
...
{
   uint32_t cashout_window_seconds = 0;                // DELTA_CASHOUT_WINDOW_SECONDS
   uint32_t reverse_auction_window_seconds = 0;        // DELTA_REVERSE_AUCTION_WINDOW_SECONDS
};

struct smt_param_vote_regeneration_period_seconds_v1
{
   uint32_t vote_regeneration_period_seconds = 0;      // DELTA_VOTE_REGENERATION_SECONDS
...
FC_REFLECT(
   delta::protocol::smt_base_operation,
   (control_account)
   (symbol)
)

FC_REFLECT(
   delta::protocol::smt_executor_base_operation,
   (executor)
   (symbol)
)

FC_REFLECT_DERIVED(
   delta::protocol::smt_create_operation,
   (delta::protocol::smt_base_operation),
   (smt_creation_fee)
   (extensions)
)
...
FC_REFLECT_DERIVED(
   delta::protocol::smt_setup_operation,
   (delta::protocol::smt_base_operation),
   (decimal_places)
   (max_supply)
...
FC_REFLECT(
   delta::protocol::smt_generation_unit,
   (delta_unit)
   (token_unit)
   )

FC_REFLECT(
   delta::protocol::smt_cap_commitment,
   (lower_bound)
   (upper_bound)
   (hash)
   )
...
FC_REFLECT(
   delta::protocol::smt_revealed_cap,
   (amount)
   (nonce)
   )

FC_REFLECT_DERIVED(
   delta::protocol::smt_cap_reveal_operation,
   (delta::protocol::smt_base_operation),
   (cap)
   (extensions)
   )
...
FC_REFLECT(
   delta::protocol::smt_capped_generation_policy,
   (pre_soft_cap_unit)
   (post_soft_cap_unit)
   (min_delta_units_commitment)
   (hard_cap_delta_units_commitment)
   (soft_cap_percent)
   (min_unit_ratio)
   (max_unit_ratio)
   (extensions)
...
FC_REFLECT_DERIVED(
   delta::protocol::smt_refund_operation,
   (delta::protocol::smt_executor_base_operation),
   (contributor)
   (contribution_id)
   (amount)
...
FC_REFLECT(
   delta::protocol::smt_emissions_unit,
   (token_unit)
   )

FC_REFLECT_DERIVED(
   delta::protocol::smt_setup_emissions_operation,
   (delta::protocol::smt_base_operation),
   (schedule_time)
   (emissions_unit)
   (interval_seconds)
...
FC_REFLECT(
   delta::protocol::smt_param_allow_vesting,
   (value)
   )

FC_REFLECT(
   delta::protocol::smt_param_allow_voting,
   (value)
   )

FC_REFLECT_TYPENAME( delta::protocol::smt_setup_parameter )

FC_REFLECT(
   delta::protocol::smt_param_windows_v1,
   (cashout_window_seconds)
   (reverse_auction_window_seconds)
   )

FC_REFLECT(
   delta::protocol::smt_param_vote_regeneration_period_seconds_v1,
   (vote_regeneration_period_seconds)
   (votes_per_regeneration_period)
   )

FC_REFLECT(
   delta::protocol::smt_param_rewards_v1,
   (content_constant)
...
FC_REFLECT_TYPENAME(
   delta::protocol::smt_runtime_parameter
   )

FC_REFLECT_DERIVED(
   delta::protocol::smt_set_setup_parameters_operation,
   (delta::protocol::smt_base_operation),
   (setup_parameters)
   (extensions)
   )
...
FC_REFLECT_DERIVED(
   delta::protocol::smt_set_runtime_parameters_operation,
   (delta::protocol::smt_base_operation),
   (runtime_parameters)
   (extensions)
   )
