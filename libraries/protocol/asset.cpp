...
{
   static_assert( DELTA_ASSET_SYMBOL_NAI_STRING_LENGTH >= 12, "This code will overflow a short buffer" );
   uint32_t x = to_nai();
   buf[11] = '\0';
   buf[10] = ((x%10)+'0');  x /= 10;
   buf[ 9] = ((x%10)+'0');  x /= 10;
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   {
      case DELTA_NAI_DELTA:
         FC_ASSERT( decimal_places == DELTA_PRECISION_DELTA );
         return DELTA_ASSET_NUM_DELTA;
      case DELTA_NAI_DBD:
         FC_ASSERT( decimal_places == DELTA_PRECISION_DBD );
         return DELTA_ASSET_NUM_DBD;
      case DELTA_NAI_VESTS:
         FC_ASSERT( decimal_places == DELTA_PRECISION_VESTS );
         return DELTA_ASSET_NUM_VESTS;
      default:
         FC_ASSERT( decimal_places <= DELTA_ASSET_MAX_DECIMALS, "Invalid decimal_places" );
...
uint32_t asset_symbol_type::to_nai()const
{
   ...
   switch( asset_num )
   {
      case DELTA_ASSET_NUM_DELTA:
         nai_data_digits = DELTA_NAI_DELTA;
         break;
      case DELTA_ASSET_NUM_DBD:
         nai_data_digits = DELTA_NAI_DBD;
         break;
      case DELTA_ASSET_NUM_VESTS:
         nai_data_digits = DELTA_NAI_VESTS;
         break;
      default:
...
asset_symbol_type asset_symbol_type::get_paired_symbol() const
{
   switch( space() )
   {
      case legacy_space:
      {
         ...
         {
            case DELTA_ASSET_NUM_DELTA:
               return from_asset_num( DELTA_ASSET_NUM_VESTS );
            case DELTA_ASSET_NUM_DBD:
               return *this;
            case DELTA_ASSET_NUM_VESTS:
               return from_asset_num( DELTA_ASSET_NUM_DELTA );
            default:
               FC_ASSERT( false, "Unknown asset symbol" );
         }
      }
...
void asset::validate()const
{
   ...
   FC_ASSERT( amount.value <= DELTA_MAX_SATOSHIS );
}
...
DEFINE_PRICE_COMPARISON_OPERATOR( >= )
      ...
      price price::max( asset_symbol_type base, asset_symbol_type quote ) { return asset( share_type(DELTA_MAX_SATOSHIS), base ) / asset( share_type(1), quote); }
      price price::min( asset_symbol_type base, asset_symbol_type quote ) { return asset( 1, base ) / asset( DELTA_MAX_SATOSHIS, quote); }
...
namespace fc {
   void to_variant( const delta::protocol::asset& var, fc::variant& vo )
   {
      try
      {
   ...
   void from_variant( const fc::variant& var, delta::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = delta::protocol::asset_symbol_type::from_nai_string( v_object[ "nai" ].as< std::string >().c_str(), v_object[ "precision" ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
