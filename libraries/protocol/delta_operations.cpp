...
namespace delta { namespace protocol {
   ...
   void validate_auth_size( const authority& a )
   {
      ...
      FC_ASSERT( size <= DELTA_MAX_AUTHORITY_MEMBERSHIP, "Authority membership exceeded. Max: 10 Current: ${n}", ("n", size) );
   }

   void account_create_operation::validate() const
   {
      validate_account_name( new_account_name );
      FC_ASSERT( is_asset_type( fee, DELTA_SYMBOL ), "Account creation fee must be DELTA" );
      owner.validate();
      active.validate();
      ...
      FC_ASSERT( fee >= asset( 0, DELTA_SYMBOL ), "Account creation fee cannot be negative" );
   }
   ...
   void comment_payout_beneficiaries::validate()const
   {
      ...
      FC_ASSERT( beneficiaries[0].weight <= DELTA_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
      sum += beneficiaries[0].weight;
      FC_ASSERT( sum <= DELTA_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

      for( size_t i = 1; i < beneficiaries.size(); i++ )
      {
         validate_account_name( beneficiaries[i].account );
         FC_ASSERT( beneficiaries[i].weight <= DELTA_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_delta_dollars <= DELTA_100_PERCENT, "Percent cannot exceed 100%" );
   ...
   void claim_account_operation::validate()const
   {
      ...
      FC_ASSERT( is_asset_type( fee, DELTA_SYMBOL ), "Account creation fee must be DELTA" );
      FC_ASSERT( fee >= asset( 0, DELTA_SYMBOL ), "Account creation fee cannot be negative" );
      FC_ASSERT( fee <= asset( DELTA_MAX_ACCOUNT_CREATION_FEE, DELTA_SYMBOL ), "Account creation fee cannot be too large" );
   ...
   void vote_operation::validate() const
   {
      ...
      validate_account_name( author );\
      FC_ASSERT( abs(weight) <= DELTA_100_PERCENT, "Weight is not a DELTAIT percentage" );
      validate_permlink( permlink );
   }
   ...
   { try {
      ...
      FC_ASSERT( amount.symbol != VESTS_SYMBOL, "transferring of DeltaX (STMP) is not allowed." );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
      FC_ASSERT( memo.size() < DELTA_MAX_MEMO_SIZE, "Memo is too large" );
   ...
   void transfer_to_vesting_operation::validate() const
   {
      ...
      FC_ASSERT( amount.symbol == DELTA_SYMBOL ||
                 ...
                 "Amount must be DELTA or SMT liquid" );
   ...
   void set_withdraw_vesting_route_operation::validate() const
   {
      ...
      FC_ASSERT( 0 <= percent && percent <= DELTA_100_PERCENT, "Percent must be valid delta percent" );
   }
   ...
   void witness_update_operation::validate() const
   {
      ...
      FC_ASSERT( url.size() <= DELTA_MAX_WITNESS_URL_LENGTH, "URL is too long" );

      FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
      FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
      FC_ASSERT( fee >= asset( 0, DELTA_SYMBOL ), "Fee cannot be negative" );
   ...
   void witness_set_properties_operation::validate() const
   {
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_creation_fee.symbol == DELTA_SYMBOL, "account_creation_fee must be in DELTA" );
         FC_ASSERT( account_creation_fee.amount >= DELTA_MIN_ACCOUNT_CREATION_FEE, "account_creation_fee smaller than minimum account creation fee" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( maximum_block_size >= DELTA_MIN_BLOCK_SIZE_LIMIT, "maximum_block_size smaller than minimum max block size" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( dbd_interest_rate <= DELTA_100_PERCENT, "dbd_interest_rate must not exceed 100%" );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( ( is_asset_type( dbd_exchange_rate.base, DBD_SYMBOL ) && is_asset_type( dbd_exchange_rate.quote, DELTA_SYMBOL ) ),
            "Price feed must be a DELTA/DBD price" );
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_budget >= DELTA_RD_MIN_BUDGET, "Budget must be at least ${n}", ("n", DELTA_RD_MIN_BUDGET) );
         FC_ASSERT( account_subsidy_budget <= DELTA_RD_MAX_BUDGET, "Budget must be at most ${n}", ("n", DELTA_RD_MAX_BUDGET) );
      }
      ...
      if( itr != props.end() )
      {
         ...
         FC_ASSERT( account_subsidy_decay >= DELTA_RD_MIN_DECAY, "Decay must be at least ${n}", ("n", DELTA_RD_MIN_DECAY) );
         FC_ASSERT( account_subsidy_decay <= DELTA_RD_MAX_DECAY, "Decay must be at most ${n}", ("n", DELTA_RD_MAX_DECAY) );
      }
   }
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( DELTA_EQUIHASH_N, DELTA_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == DELTA_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == DELTA_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   void feed_publish_operation::validate()const
   {
      ...
      FC_ASSERT( ( is_asset_type( exchange_rate.base, DELTA_SYMBOL ) && is_asset_type( exchange_rate.quote, DBD_SYMBOL ) )
         || ( is_asset_type( exchange_rate.base, DBD_SYMBOL ) && is_asset_type( exchange_rate.quote, DELTA_SYMBOL ) ),
         "Price feed must be a DELTA/DBD price" );
   ...
   void limit_order_create_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, DELTA_SYMBOL ) && is_asset_type( min_to_receive, DBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, DBD_SYMBOL ) && is_asset_type( min_to_receive, DELTA_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( min_to_receive, DELTA_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, DELTA_SYMBOL )
                     && min_to_receive.symbol.space() == asset_symbol_type::smt_nai_space
                  ),
               "Limit order must be for the DELTA:DBD or SMT:(DELTA/DBD) market" );
   ...
   void limit_order_create2_operation::validate()const
   {
      ...
      FC_ASSERT(  ( is_asset_type( amount_to_sell, DELTA_SYMBOL ) && is_asset_type( exchange_rate.quote, DBD_SYMBOL ) )
               || ( is_asset_type( amount_to_sell, DBD_SYMBOL ) && is_asset_type( exchange_rate.quote, DELTA_SYMBOL ) )
               || (
                     amount_to_sell.symbol.space() == asset_symbol_type::smt_nai_space
                     && is_asset_type( exchange_rate.quote, DELTA_SYMBOL )
                  )
               || (
                     is_asset_type( amount_to_sell, DELTA_SYMBOL )
   ...
   {
      validate_account_name( owner );
      /// only allow conversion from DBD to DELTA, allowing the opposite can enable traders to abuse
      /// market fluxuations through converting large quantities without moving the price.
      FC_ASSERT( is_asset_type( amount, DBD_SYMBOL ), "Can only convert DBD to DELTA" );
   ...
   void escrow_transfer_operation::validate()const
   {
      ...
      FC_ASSERT( delta_amount.amount >= 0, "delta amount cannot be negative" );
      FC_ASSERT( dbd_amount.amount > 0 || delta_amount.amount > 0, "escrow must transfer a non-zero amount" );
      FC_ASSERT( from != agent && to != agent, "agent must be a third party" );
      FC_ASSERT( (fee.symbol == DELTA_SYMBOL) || (fee.symbol == DBD_SYMBOL), "fee must be DELTA or DBD" );
      FC_ASSERT( dbd_amount.symbol == DBD_SYMBOL, "dbd amount must contain DBD" );
      FC_ASSERT( delta_amount.symbol == DELTA_SYMBOL, "delta amount must contain DELTA" );
   ...
   void escrow_release_operation::validate()const
   {
      ...
      FC_ASSERT( dbd_amount.amount >= 0, "dbd amount cannot be negative" );
      FC_ASSERT( delta_amount.amount >= 0, "delta amount cannot be negative" );
      FC_ASSERT( dbd_amount.amount > 0 || delta_amount.amount > 0, "escrow must release a non-zero amount" );
      FC_ASSERT( dbd_amount.symbol == DBD_SYMBOL, "dbd amount must contain DBD" );
      FC_ASSERT( delta_amount.symbol == DELTA_SYMBOL, "delta amount must contain DELTA" );
   }
   ...
   void transfer_to_savings_operation::validate()const {
      ...
      FC_ASSERT( amount.symbol == DELTA_SYMBOL || amount.symbol == DBD_SYMBOL );
      FC_ASSERT( memo.size() < DELTA_MAX_MEMO_SIZE, "Memo is too large" );
      FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
   }
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_delta, DELTA_SYMBOL ), "Reward Delta must be DELTA" );
      FC_ASSERT( is_asset_type( reward_dbd, DBD_SYMBOL ), "Reward Delta must be DBD" );
      FC_ASSERT( is_asset_type( reward_vests, VESTS_SYMBOL ), "Reward Delta must be VESTS" );
      FC_ASSERT( reward_delta.amount >= 0, "Cannot claim a negative amount" );
      ...
      FC_ASSERT( reward_delta.amount > 0 || reward_dbd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
   }
