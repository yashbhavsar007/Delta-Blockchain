...
#include <delta/chain/compound.hpp>
#include <delta/chain/custom_operation_interpreter.hpp>
#include <delta/chain/database.hpp>
#include <delta/chain/database_exceptions.hpp>
#include <delta/chain/db_with.hpp>
#include <delta/chain/evaluator_registry.hpp>
#include <delta/chain/global_property_object.hpp>
#include <delta/chain/history_object.hpp>
#include <delta/chain/index.hpp>
#include <delta/chain/pending_required_action_object.hpp>
#include <delta/chain/pending_optional_action_object.hpp>
...
#include <delta/chain/shared_db_merkle.hpp>
#include <delta/chain/witness_schedule.hpp>

#include <delta/chain/util/asset.hpp>
#include <delta/chain/util/reward.hpp>
#include <delta/chain/util/uint256.hpp>
#include <delta/chain/util/reward.hpp>
#include <delta/chain/util/manabar.hpp>
#include <delta/chain/util/rd_setup.hpp>
...
FC_REFLECT( delta::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( delta::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( delta::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
struct reward_fund_context
{
   ...
   asset       reward_balance = asset( 0, DELTA_SYMBOL );
   share_type  delta_awarded = 0;
};
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&note) {
      DELTA_TRY_NOTIFY(this_->_post_reindex_signal, note);
   } BOOST_SCOPE_EXIT_END

   try
   {
      DELTA_TRY_NOTIFY(_pre_reindex_signal, note);
      ...
      DELTA_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
   return delta_chain_id;
}

void database::set_chain_id( const chain_id_type& chain_id )
{
   delta_chain_id = chain_id;

   idump( (delta_chain_id) );
}
...
{ try {
   if( !has_hardfork( DELTA_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( DELTA_HARDFORK_0_17__769 ) || comment.parent_author == DELTA_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(DELTA_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   if( has_hardfork( DELTA_HARDFORK_0_5__54 ) )
   {
      const auto& witness = get_witness( witness_owner );

      if( witness.running_version != DELTA_BLOCKCHAIN_VERSION )
         pending_block.extensions.insert( block_header_extensions( DELTA_BLOCKCHAIN_VERSION ) );

      const auto& hfp = get_hardfork_property_object();

      if( hfp.current_hardfork_version < DELTA_BLOCKCHAIN_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == DELTA_BLOCKCHAIN_VERSION // Binary does not know of a new hardfork
         && witness.hardfork_version_vote > DELTA_BLOCKCHAIN_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //DELTA_MAX_BLOCK_SIZE;
   ...
   if( has_hardfork( DELTA_HARDFORK_0_20 ) )
   {
   ...
   if( !(skip & skip_witness_signature) )
      pending_block.sign( block_signing_private_key, has_hardfork( DELTA_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );

   // TODO:  Move this to _push_block() so session is restored.
   if( !(skip & skip_block_size_check) )
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= DELTA_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      DELTA_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
{
   DELTA_TRY_NOTIFY( _pre_apply_operation_signal, note )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = DELTA_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / DELTA_BLOCK_INTERVAL + 1;
}

/**
 *  Converts DELTA into dbd and adds it to to_account while reducing the DELTA supply
 *  by DELTA and increasing the dbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_dbd( const account_object& to_account, asset delta, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, DBD_SYMBOL ), asset( 0, DELTA_SYMBOL ) );
   ...
   try
   {
      ...
      {
         auto to_dbd = ( gpo.dbd_print_rate * delta.amount ) / DELTA_100_PERCENT;
         auto to_delta = delta.amount - to_dbd;

         auto dbd = asset( to_dbd, DELTA_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, dbd );
            adjust_reward_balance( to_account, asset( to_delta, DELTA_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, dbd );
            adjust_balance( to_account, asset( to_delta, DELTA_SYMBOL ) );
         }

         adjust_supply( asset( -to_dbd, DELTA_SYMBOL ) );
         adjust_supply( dbd );
         assets.first = dbd;
         assets.second = asset( to_delta, DELTA_SYMBOL );
      }
      else
      {
         adjust_balance( to_account, delta );
         assets.second = delta;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(delta) )
...
asset create_vesting2( database& db, const account_object& to_account, asset liquid, bool to_reward_balance, Before&& before_vesting_callback )
{
   try
   {
      auto calculate_new_vesting = [ liquid ] ( price vesting_share_price ) -> asset
         {
         /**
          *  The ratio of total_vesting_shares / total_vesting_fund_delta should not
      ...
      FC_ASSERT( liquid.symbol == DELTA_SYMBOL );
      ...
      {
         if( db.has_hardfork( DELTA_HARDFORK_0_20__2539 ) )
         {
            db.modify( to_account, [&]( account_object& a )
            {
               util::manabar_params params( util::get_effective_vesting_shares( a ), DELTA_VOTING_MANA_REGENERATION_SECONDS );
FC_TODO( "Set skip_cap_regen=true without breaking consensus" );
      ...
      db.modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_delta += liquid;
         }
         else
         {
            props.total_vesting_fund_delta += liquid;
 ...
 * @param liquid     - DELTA or liquid SMT to be converted to vesting shares
 */
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( DELTA_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, DELTA_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != DELTA_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= DELTA_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = DELTA_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = DELTA_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( DELTA_HARDFORK_0_2 ) )
         w.virtual_scheduled_time = w.virtual_last_update + (DELTA_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
      else
         w.virtual_scheduled_time = w.virtual_last_update + (DELTA_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

      /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
      if( has_hardfork( DELTA_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( DELTA_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( DELTA_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( DELTA_NULL_ACCOUNT );
   asset total_delta( 0, DELTA_SYMBOL );
   asset total_dbd( 0, DBD_SYMBOL );
   asset total_vests( 0, VESTS_SYMBOL );

   asset vesting_shares_delta_value = asset( 0, DELTA_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_delta += null_account.balance;
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_delta += null_account.savings_balance;
   }
   ...
   {
      const auto& gpo = get_dynamic_global_properties();
      vesting_shares_delta_value = null_account.vesting_shares * gpo.get_vesting_share_price();
      total_delta += vesting_shares_delta_value;
      total_vests += null_account.vesting_shares;
   }

   if( null_account.reward_delta_balance.amount > 0 )
   {
      total_delta += null_account.reward_delta_balance;
   }
   ...
   {
      total_delta += null_account.reward_vesting_delta;
      total_vests += null_account.reward_vesting_balance;
   }

   if( (total_delta.amount.value == 0) && (total_dbd.amount.value == 0) && (total_vests.amount.value == 0) )
      return;
   ...
   if( total_delta.amount.value > 0 )
      vop.total_cleared.push_back( total_delta );
   ...
   if( null_account.vesting_shares.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.total_vesting_fund_delta -= vesting_shares_delta_value;
      });
   ...
   if( null_account.reward_delta_balance.amount > 0 )
   {
      adjust_reward_balance( null_account, -null_account.reward_delta_balance );
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.pending_rewarded_vesting_delta -= null_account.reward_vesting_delta;
      });

      modify( null_account, [&]( account_object& a )
      {
         a.reward_vesting_delta.amount = 0;
...
{
   if( head_block_num() >= DELTA_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_delta = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_delta_converted = asset( 0, DELTA_SYMBOL );

      // Do two passes, the first for vests, the second for delta. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / DELTA_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / DELTA_100_PERCENT ).to_uint64();
            vests_deposited_as_delta += to_deposit;
            auto converted_delta = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_delta_converted += converted_delta;

            if( to_deposit > 0 )
            {
               operation vop = fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_delta );

               pre_push_virtual_operation( vop );

               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_delta;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_delta -= converted_delta;
      ...
      share_type to_convert = to_withdraw - vests_deposited_as_delta - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_delta = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      operation vop = fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_delta );
      pre_push_virtual_operation( vop );
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         for( auto& item : proxy_set )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               create_vesting2( *this, voter, asset( claim, DELTA_SYMBOL ), has_hardfork( DELTA_HARDFORK_0_17__659 ),
                  [&]( const asset& reward )
                  {
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment, bool forward_curation_remainder )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( DELTA_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / DELTA_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / DELTA_100_PERCENT;
               auto benefactor_vesting_delta = benefactor_tokens;
               auto vop = comment_benefactor_reward_operation( b.account, comment.author, to_string( comment.permlink ), asset( 0, DBD_SYMBOL ), asset( 0, DELTA_SYMBOL ), asset( 0, VESTS_SYMBOL ) );

               if( has_hardfork( DELTA_HARDFORK_0_20__2022 ) )
               {
                  auto benefactor_dbd_delta = ( benefactor_tokens * comment.percent_delta_dollars ) / ( 2 * DELTA_100_PERCENT ) ;
                  benefactor_vesting_delta  = benefactor_tokens - benefactor_dbd_delta;
                  auto dbd_payout           = create_dbd( get_account( b.account ), asset( benefactor_dbd_delta, DELTA_SYMBOL ), true );

                  vop.dbd_payout   = dbd_payout.first; // DBD portion
                  vop.delta_payout = dbd_payout.second; // DELTA portion
               }

               create_vesting2( *this, get_account( b.account ), asset( benefactor_vesting_delta, DELTA_SYMBOL ), has_hardfork( DELTA_HARDFORK_0_17__659 ),
               [&]( const asset& reward )
               {
            ...
            auto dbd_delta     = ( author_tokens * comment.percent_delta_dollars ) / ( 2 * DELTA_100_PERCENT ) ;
            auto vesting_delta = author_tokens - dbd_delta;

            const auto& author = get_account( comment.author );
            auto dbd_payout = create_dbd( author, asset( dbd_delta, DELTA_SYMBOL ), has_hardfork( DELTA_HARDFORK_0_17__659 ) );
            ...
            create_vesting2( *this, author, asset( vesting_delta, DELTA_SYMBOL ), has_hardfork( DELTA_HARDFORK_0_17__659 ),
            ...
            adjust_total_payout( comment, dbd_payout.first + to_dbd( dbd_payout.second + asset( vesting_delta, DELTA_SYMBOL ) ), to_dbd( asset( curation_tokens, DELTA_SYMBOL ) ), to_dbd( asset( total_beneficiary, DELTA_SYMBOL ) ) );

            post_push_virtual_operation( vop );
            vop = comment_reward_operation( comment.author, to_string( comment.permlink ), to_dbd( asset( claimed_reward, DELTA_SYMBOL ) ) );
            pre_push_virtual_operation( vop );
            post_push_virtual_operation( vop );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( DELTA_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == DELTA_ROOT_POST_PARENT )
         {
            if( has_hardfork( DELTA_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + DELTA_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( DELTA_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( DELTA_FIRST_CASHOUT_TIME ) )
      return;

   const auto& gpo = get_dynamic_global_properties();
   util::comment_reward_context ctx;
   ctx.current_delta_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > delta_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( DELTA_HARDFORK_0_19__1051 ) )
            decay_time = DELTA_RECENT_RSHARES_DECAY_TIME_HF19;
         else
            decay_time = DELTA_RECENT_RSHARES_DECAY_TIME_HF17;
   ...
   if( has_hardfork( DELTA_HARDFORK_0_17__771 ) )
   {
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( DELTA_HARDFORK_0_17__771 ) )
      {
         ...
         ctx.total_reward_fund_delta = funds[ fund_id ].reward_balance;

         bool forward_curation_remainder = !has_hardfork( DELTA_HARDFORK_0_20__1877 );

         funds[ fund_id ].delta_awarded += cashout_comment_helper( ctx, *current, forward_curation_remainder );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.total_reward_fund_delta = gpo.total_reward_fund_delta;
            ...
            if( reward > 0 )
            {
               ...
               {
                  p.total_reward_fund_delta.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= asset( funds[ i ].delta_awarded, DELTA_SYMBOL );
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual delta per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( DELTA_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( DELTA_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / DELTA_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( DELTA_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_delta = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( DELTA_100_PERCENT ) * int64_t( DELTA_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_delta * DELTA_CONTENT_REWARD_PERCENT ) / DELTA_100_PERCENT;
      if( has_hardfork( DELTA_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward ); /// 75% to content creator
      auto vesting_reward = ( new_delta * DELTA_VESTING_FUND_PERCENT ) / DELTA_100_PERCENT; /// 15% to vesting fund
      auto witness_reward = new_delta - content_reward - vesting_reward; /// Remaining 10% to witness pay

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= DELTA_MAX_WITNESSES;
      ...
      new_delta = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_delta += asset( vesting_reward, DELTA_SYMBOL );
         if( !has_hardfork( DELTA_HARDFORK_0_17__774 ) )
            p.total_reward_fund_delta  += asset( content_reward, DELTA_SYMBOL );
         p.current_supply           += asset( new_delta, DELTA_SYMBOL );
         p.virtual_supply           += asset( new_delta, DELTA_SYMBOL );
      });

      operation vop = producer_reward_operation( cwit.owner, asset( 0, VESTS_SYMBOL ) );
      create_vesting2( *this, get_account( cwit.owner ), asset( witness_reward, DELTA_SYMBOL ), false,
   ...
   else
   {
      ...
      if( props.head_block_number < DELTA_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_delta += vesting_reward;
          p.total_reward_fund_delta  += content_reward;
...
void process_smt_objects_internal( database* db, delta::chain::smt_phase phase )
{
...
{
   if( has_hardfork( DELTA_HARDFORK_0_12__178 ) )
      return asset( 0, DELTA_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( DELTA_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< DELTA_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), DELTA_SYMBOL );
   return std::max( percent, DELTA_MIN_LIQUIDITY_REWARD );
}

asset database::get_content_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( DELTA_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< DELTA_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), DELTA_SYMBOL );
   return std::max( percent, DELTA_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
   const auto& props = get_dynamic_global_properties();
   static_assert( DELTA_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< DELTA_CURATE_APR_PERCENT >( props.virtual_supply.amount ), DELTA_SYMBOL);
   return std::max( percent, DELTA_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
   const auto& props = get_dynamic_global_properties();
   static_assert( DELTA_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   asset percent( protocol::calc_percent_reward_per_block< DELTA_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), DELTA_SYMBOL);
   auto pay = std::max( percent, DELTA_MIN_PRODUCER_REWARD );
   const auto& witness_account = get_account( props.current_witness );

   /// pay witness in vesting shares
   if( props.head_block_number >= DELTA_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) )
   {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least DELTA_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < DELTA_MAX_WITNESSES && props.head_block_number < DELTA_START_VESTING_BLOCK )
      return asset( 0, DELTA_SYMBOL );
#endif

   static_assert( DELTA_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
   static_assert( DELTA_MAX_WITNESSES == 21, "this code assumes 21 per round" );
   asset percent( calc_percent_reward_per_round< DELTA_POW_APR_PERCENT >( props.virtual_supply.amount ), DELTA_SYMBOL);
   return std::max( percent, DELTA_MIN_POW_REWARD );
}
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % DELTA_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( DELTA_HARDFORK_0_8__116 ) )
      return DELTA_1_PERCENT * 25;
   else
      return DELTA_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / DELTA_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, DELTA_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more DELTA than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from delta/dbd at the
...
void database::process_conversions()
{
   ...
   asset net_delta( 0, DELTA_SYMBOL );
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_delta;
       p.current_dbd_supply -= net_dbd;
       p.virtual_supply += net_delta;
       p.virtual_supply -= net_dbd * get_feed_history().current_median_history;
   } );
}

asset database::to_dbd( const asset& delta )const
{
   return util::to_dbd( get_feed_history().current_median_history, delta );
}

asset database::to_delta( const asset& dbd )const
{
   return util::to_delta( get_feed_history().current_median_history, dbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + DELTA_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( old_escrow.from, old_escrow.delta_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, DELTA_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < DELTA_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = DELTA_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(DELTA_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = DELTA_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = DELTA_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = DELTA_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = DELTA_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = DELTA_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = DELTA_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < DELTA_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = DELTA_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, DELTA_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = DELTA_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = DELTA_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = DELTA_INIT_MINER_NAME;
         p.time = DELTA_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, DELTA_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = DELTA_MAX_BLOCK_SIZE;
         p.reverse_auction_seconds = DELTA_REVERSE_AUCTION_WINDOW_SECONDS_HF6;
         p.dbd_stop_percent = DELTA_DBD_STOP_PERCENT_HF14;
         p.dbd_start_percent = DELTA_DBD_START_PERCENT_HF14;
      } );
      ...
      {
         hpo.processed_hardforks.push_back( DELTA_GENESIS_TIME );
      } );
      ...
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         ...
         wso.current_shuffled_witnesses[0] = DELTA_INIT_MINER_NAME;
         util::rd_system_params account_subsidy_system_params;
         account_subsidy_system_params.resource_unit = DELTA_ACCOUNT_SUBSIDY_PRECISION;
         account_subsidy_system_params.decay_per_time_unit_denom_shift = DELTA_RD_DECAY_DENOM_SHIFT;
         ...
         w_budget = (w_budget * DELTA_WITNESS_SUBSIDY_BUDGET_PERCENT) / DELTA_100_PERCENT;
         w_budget = std::min( w_budget, int64_t(std::numeric_limits<int32_t>::max()) );
         uint64_t w_decay = wso.median_props.account_subsidy_decay;
         w_decay = (w_decay * DELTA_WITNESS_SUBSIDY_DECAY_PERCENT) / DELTA_100_PERCENT;
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      DELTA_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         DELTA_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
   ...
   if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( DELTA_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / DELTA_100_PERCENT ).to_uint64() ) )
   {
      uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / DELTA_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
   ...
   if( BOOST_UNLIKELY( next_block_num == 1 ) )
   {
      ...
      uint32_t n;
      for( n=0; n<DELTA_NUM_HARDFORKS; n++ )
      {
   ...
   if( has_hardfork( DELTA_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < DELTA_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",DELTA_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( DELTA_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
struct process_header_visitor
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( DELTA_SMT_HARDFORK ), "Automated actions are not enabled until SMT hardfork." );
   }
...
try {
   if( (head_block_num() % DELTA_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( DELTA_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_dbd_exchange_update + DELTA_MAX_FEED_AGE_SECONDS
            && !wit.dbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.dbd_exchange_rate );
         }
      }
      else if( wit.last_dbd_exchange_update < now + DELTA_MAX_FEED_AGE_SECONDS &&
          !wit.dbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.dbd_exchange_rate );
      }
   ...
   if( feeds.size() >= DELTA_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t delta_feed_history_window = DELTA_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( DELTA_HARDFORK_0_16__551) )
            delta_feed_history_window = DELTA_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > delta_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( DELTA_HARDFORK_0_14__230 ) )
            {
               // This block limits the effective median price to force DBD to remain at or
               // below 10% of the combined market cap of DELTA and DBD.
               //
               // For example, if we have 500 DELTA and 100 DBD, the price is limited to
               // 900 DBD / 500 DELTA which works out to be $1.80.  At this price, 500 Delta
...
{ try {
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, DELTA_MAX_SIG_CHECK_DEPTH,
            has_hardfork( DELTA_HARDFORK_0_20 ) || is_producing() ? DELTA_MAX_AUTHORITY_MEMBERSHIP : 0,
            has_hardfork( DELTA_HARDFORK_0_20 ) || is_producing() ? DELTA_MAX_SIG_CHECK_ACCOUNTS : 0,
            has_hardfork( DELTA_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         DELTA_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      DELTA_ASSERT( trx.expiration <= now + fc::seconds(DELTA_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",DELTA_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( DELTA_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         DELTA_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      DELTA_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
FC_TODO( "#ifndef not needed after HF 20 is live" );
#ifndef IS_TEST_NET
               if( has_hardfork( DELTA_HARDFORK_0_14__278 ) && !has_hardfork( DELTA_HARDFORK_0_20__SP190 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > DELTA_BLOCKS_PER_DAY )
                  {
   ...
   {
      DELTA_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < DELTA_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",DELTA_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, DELTA_SYMBOL ) : dgp.current_dbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( DELTA_HARDFORK_0_14__230 ) )
      {
         auto percent_dbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_dbd_supply * get_feed_history().current_median_history ).amount.value ) * DELTA_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_dbd <= dgp.dbd_start_percent )
            dgp.dbd_print_rate = DELTA_100_PERCENT;
         else if( percent_dbd >= dgp.dbd_stop_percent )
            dgp.dbd_print_rate = 0;
         else
            dgp.dbd_print_rate = ( ( dgp.dbd_stop_percent - percent_dbd ) * DELTA_100_PERCENT ) / ( dgp.dbd_stop_percent - dgp.dbd_start_percent );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < DELTA_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > DELTA_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - DELTA_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( DELTA_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((DELTA_100_PERCENT - DELTA_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / DELTA_100_PERCENT);
...
{
   bool has_hf_20__1815 = has_hardfork( DELTA_HARDFORK_0_20__1815 );

#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      DELTA_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
      ...
      DELTA_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      DELTA_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      DELTA_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      DELTA_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   }
...
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      DELTA_ASSERT( new_order_pays == new_order.amount_for_sale() ||
   ...
   if( !has_hardfork( DELTA_HARDFORK_0_12__178 ) &&
       ( (age >= DELTA_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( DELTA_HARDFORK_0_10__149)) ||
       (age >= DELTA_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( DELTA_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == DELTA_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= DELTA_LIQUIDITY_TIMEOUT_SEC )
         {
            r.dbd_volume = 0;
            r.delta_volume = 0;
            r.weight = 0;
         ...
         if( is_sdb )
            ...
            r.delta_volume += volume.amount.value;

         r.update_weight( has_hardfork( DELTA_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( DELTA_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
   try
   {
      ...
      {
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
         if( has_hardfork( DELTA_HARDFORK_0_20__1815 ) )
         {
            DELTA_ASSERT( pays < order.amount_for_sale(),
              order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
              ("order", order)("pays", pays)("receives", receives) );
         }
...
void database::clear_expired_delegations()
{
   ...
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      ...
      {
         if( has_hardfork( DELTA_HARDFORK_0_20__2539 ) )
         {
            util::manabar_params params( util::get_effective_vesting_shares( a ), DELTA_VOTING_MANA_REGENERATION_SECONDS );
FC_TODO( "Set skip_cap_regen=true without breaking consensus" );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case DELTA_ASSET_NUM_DELTA:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.balance.amount.value >= 0, "Insufficient DELTA funds" );
            }
            break;
         case DELTA_ASSET_NUM_DBD:
            if( a.dbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.dbd_seconds > 0 &&
                   (acnt.dbd_seconds_last_update - acnt.dbd_last_interest_payment).to_seconds() > DELTA_DBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.dbd_seconds / DELTA_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().dbd_interest_rate;
                  interest /= DELTA_100_PERCENT;
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( value_delta.symbol.asset_num )
      {
         case DELTA_ASSET_NUM_DELTA:
            ...
            {
               acnt.reward_delta_balance += value_delta;
               if( check_balance )
               {
                  FC_ASSERT( acnt.reward_delta_balance.amount.value >= 0, "Insufficient reward DELTA funds" );
               }
            }
            else
            {
               acnt.reward_vesting_delta += value_delta;
...
{
   bool check_balance = has_hardfork( DELTA_HARDFORK_0_20__1811 );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case DELTA_ASSET_NUM_DELTA:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_balance.amount.value >= 0, "Insufficient savings DELTA funds" );
            }
            break;
         case DELTA_ASSET_NUM_DBD:
            if( a.savings_dbd_seconds_last_update != head_block_time() )
            {
               ...
               if( acnt.savings_dbd_seconds > 0 &&
                   (acnt.savings_dbd_seconds_last_update - acnt.savings_dbd_last_interest_payment).to_seconds() > DELTA_DBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_dbd_seconds / DELTA_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().dbd_interest_rate;
                  interest /= DELTA_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), DBD_SYMBOL);
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   bool check_supply = has_hardfork( DELTA_HARDFORK_0_20__1811 );

   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < DELTA_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol.asset_num )
      {
         case DELTA_ASSET_NUM_DELTA:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, DELTA_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_delta += new_vesting;
            if( check_supply )
            {
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( DELTA_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( DELTA_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_1 ] = fc::time_point_sec( DELTA_HARDFORK_0_1_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_1 ] = DELTA_HARDFORK_0_1_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_2 ] = fc::time_point_sec( DELTA_HARDFORK_0_2_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_2 ] = DELTA_HARDFORK_0_2_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_3 ] = fc::time_point_sec( DELTA_HARDFORK_0_3_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_3 ] = DELTA_HARDFORK_0_3_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_4 ] = fc::time_point_sec( DELTA_HARDFORK_0_4_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_4 ] = DELTA_HARDFORK_0_4_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_5 ] = fc::time_point_sec( DELTA_HARDFORK_0_5_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_5 ] = DELTA_HARDFORK_0_5_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_6 ] = fc::time_point_sec( DELTA_HARDFORK_0_6_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_6 ] = DELTA_HARDFORK_0_6_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_7 ] = fc::time_point_sec( DELTA_HARDFORK_0_7_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_7 ] = DELTA_HARDFORK_0_7_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_8 ] = fc::time_point_sec( DELTA_HARDFORK_0_8_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_8 ] = DELTA_HARDFORK_0_8_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_9 ] = fc::time_point_sec( DELTA_HARDFORK_0_9_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_9 ] = DELTA_HARDFORK_0_9_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_10 ] = fc::time_point_sec( DELTA_HARDFORK_0_10_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_10 ] = DELTA_HARDFORK_0_10_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_11 ] = fc::time_point_sec( DELTA_HARDFORK_0_11_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_11 ] = DELTA_HARDFORK_0_11_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_12 ] = fc::time_point_sec( DELTA_HARDFORK_0_12_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_12 ] = DELTA_HARDFORK_0_12_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_13 ] = fc::time_point_sec( DELTA_HARDFORK_0_13_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_13 ] = DELTA_HARDFORK_0_13_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_14 ] = fc::time_point_sec( DELTA_HARDFORK_0_14_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_14 ] = DELTA_HARDFORK_0_14_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_15 ] = fc::time_point_sec( DELTA_HARDFORK_0_15_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_15 ] = DELTA_HARDFORK_0_15_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_16 ] = fc::time_point_sec( DELTA_HARDFORK_0_16_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_16 ] = DELTA_HARDFORK_0_16_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_17 ] = fc::time_point_sec( DELTA_HARDFORK_0_17_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_17 ] = DELTA_HARDFORK_0_17_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_18 ] = fc::time_point_sec( DELTA_HARDFORK_0_18_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_18 ] = DELTA_HARDFORK_0_18_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_19 ] = fc::time_point_sec( DELTA_HARDFORK_0_19_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_19 ] = DELTA_HARDFORK_0_19_VERSION;
   FC_ASSERT( DELTA_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_20 ] = fc::time_point_sec( DELTA_HARDFORK_0_20_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_20 ] = DELTA_HARDFORK_0_20_VERSION;
#ifdef IS_TEST_NET
   FC_ASSERT( DELTA_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
   _hardfork_times[ DELTA_HARDFORK_0_21 ] = fc::time_point_sec( DELTA_HARDFORK_0_21_TIME );
   _hardfork_versions[ DELTA_HARDFORK_0_21 ] = DELTA_HARDFORK_0_21_VERSION;
#endif


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= DELTA_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("DELTA_NUM_HARDFORKS",DELTA_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= DELTA_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( DELTA_BLOCKCHAIN_HARDFORK_VERSION >= DELTA_BLOCKCHAIN_VERSION );
   FC_ASSERT( DELTA_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ DELTA_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      if( has_hardfork( DELTA_HARDFORK_0_5__54 ) )
      {
         ...
         {
            if( hardforks.last_hardfork < DELTA_NUM_HARDFORKS ) {
      ...
      {
         while( hardforks.last_hardfork < DELTA_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < DELTA_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= DELTA_NUM_HARDFORKS; i++ )
   {
      if( i <= DELTA_HARDFORK_0_5__54 )
...
void database::apply_hardfork( uint32_t hardfork )
{
   ...
   switch( hardfork )
   {
      case DELTA_HARDFORK_0_1:
         perform_vesting_share_split( 1000000 );
         break;
      case DELTA_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case DELTA_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case DELTA_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case DELTA_HARDFORK_0_5:
         break;
      case DELTA_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case DELTA_HARDFORK_0_7:
         break;
      case DELTA_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case DELTA_HARDFORK_0_9:
         {
         ...
         break;
      case DELTA_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case DELTA_HARDFORK_0_11:
         break;
      case DELTA_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == DELTA_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + DELTA_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + DELTA_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( DELTA_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( DELTA_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( DELTA_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case DELTA_HARDFORK_0_13:
         break;
      case DELTA_HARDFORK_0_14:
         break;
      case DELTA_HARDFORK_0_15:
         break;
      case DELTA_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > DELTA_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case DELTA_HARDFORK_0_17:
         {
            static_assert(
               DELTA_MAX_VOTED_WITNESSES_HF0 + DELTA_MAX_MINER_WITNESSES_HF0 + DELTA_MAX_RUNNER_WITNESSES_HF0 == DELTA_MAX_WITNESSES,
               "HF0 witness counts must add up to DELTA_MAX_WITNESSES" );
            static_assert(
               DELTA_MAX_VOTED_WITNESSES_HF17 + DELTA_MAX_MINER_WITNESSES_HF17 + DELTA_MAX_RUNNER_WITNESSES_HF17 == DELTA_MAX_WITNESSES,
               "HF17 witness counts must add up to DELTA_MAX_WITNESSES" );

            modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
            {
               wso.max_voted_witnesses = DELTA_MAX_VOTED_WITNESSES_HF17;
               wso.max_miner_witnesses = DELTA_MAX_MINER_WITNESSES_HF17;
               wso.max_runner_witnesses = DELTA_MAX_RUNNER_WITNESSES_HF17;
            });
            ...
            {
               rfo.name = DELTA_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = DELTA_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = DELTA_1_PERCENT * 25;
               rfo.percent_content_rewards = DELTA_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_delta;
#ifndef IS_TEST_NET
               rfo.recent_claims = DELTA_HF_17_RECENT_CLAIMS;
#endif
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_delta = asset( 0, DELTA_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( DELTA_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( DELTA_HF_17_NUM_REPLIES );
            ...
            for( const auto& itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + DELTA_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( const auto& itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + DELTA_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case DELTA_HARDFORK_0_18:
         break;
      case DELTA_HARDFORK_0_19:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.vote_power_reserve_rate = DELTA_REDUCED_VOTE_POWER_RATE;
            });

            modify( get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = DELTA_HF_19_RECENT_CLAIMS;
#endif
         ...
         break;
      case DELTA_HARDFORK_0_20:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.delegation_return_period = DELTA_DELEGATION_RETURN_PERIOD_HF20;
               gpo.reverse_auction_seconds = DELTA_REVERSE_AUCTION_WINDOW_SECONDS_HF20;
               gpo.dbd_stop_percent = DELTA_DBD_STOP_PERCENT_HF20;
               gpo.dbd_start_percent = DELTA_DBD_START_PERCENT_HF20;
            ...
            for( const auto& witness : wso.current_shuffled_witnesses )
            {
               ...
               if( witness != account_name_type() )
               {
                  ...
                  {
                     w.props.account_creation_fee = asset( w.props.account_creation_fee.amount * DELTA_CREATE_ACCOUNT_WITH_DELTA_MODIFIER, DELTA_SYMBOL );
                  });
               }
            }
            ...
            {
               wso.median_props.account_creation_fee = asset( wso.median_props.account_creation_fee.amount * DELTA_CREATE_ACCOUNT_WITH_DELTA_MODIFIER, DELTA_SYMBOL );
            });
         }
         break;
   #ifdef IS_TEST_NET
      case DELTA_HARDFORK_0_21:
         break;
#endif
      default:
         break;
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, DELTA_SYMBOL );
      asset total_dbd = asset( 0, DBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_delta = asset( 0, DELTA_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_delta_balance;
         ...
         pending_vesting_delta += itr->reward_vesting_delta;
         total_vsf_votes += ( itr->proxy == DELTA_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( DELTA_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[DELTA_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == DELTA_SYMBOL )
      ...
      {
         if( itr->sell_price.base.symbol == DELTA_SYMBOL )
         {
            total_supply += asset( itr->for_sale, DELTA_SYMBOL );
         }
      ...
      {
         total_supply += itr->delta_balance;
         total_dbd += itr->dbd_balance;

         if( itr->pending_fee.symbol == DELTA_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == DBD_SYMBOL )
            total_dbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not DBD or DELTA" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == DELTA_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not DBD or DELTA" );
      }
      ...
      total_supply += gpo.total_vesting_fund_delta + gpo.total_reward_fund_delta + gpo.pending_rewarded_vesting_delta;

      FC_ASSERT( gpo.current_supply == total_supply, "", ("gpo.current_supply",gpo.current_supply)("total_supply",total_supply) );
      FC_ASSERT( gpo.current_dbd_supply == total_dbd, "", ("gpo.current_dbd_supply",gpo.current_dbd_supply)("total_dbd",total_dbd) );
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_delta == pending_vesting_delta, "", ("pending_rewarded_vesting_delta",gpo.pending_rewarded_vesting_delta)("pending_vesting_delta", pending_vesting_delta));
...
void database::validate_smt_invariants()const
{
   try
   {
      ...
      // - Process reward balances, collecting SMT counterparts of 'reward_delta_balance', 'reward_vesting_balance' & 'reward_vesting_delta'.
   ...
   try
   {
      ...
      for( const auto& account : get_index<account_index>().indices() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / DELTA_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < DELTA_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != DELTA_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != DELTA_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != DELTA_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.name )
