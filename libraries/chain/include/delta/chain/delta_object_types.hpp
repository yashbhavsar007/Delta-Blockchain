...
using delta::protocol::chain_id_type;
...
{
   class variant;
   inline void to_variant( const delta::chain::shared_string& s, variant& var )
   {
      var = fc::string( delta::chain::to_string( s ) );
   }

   inline void from_variant( const variant& var, delta::chain::shared_string& s )
   {
   ...
   namespace raw {
      ...
      template< typename T >
      inline T unpack_from_vector( const delta::chain::buffer_type& s )
      { try  {
         T tmp;
         if( s.size() ) {
...
FC_REFLECT_ENUM( delta::chain::object_type,
                 (dynamic_global_property_object_type)
                 (account_object_type)
                 (account_authority_object_type)
                 (witness_object_type)
...
#ifndef ENABLE_STD_ALLOCATOR
FC_REFLECT_TYPENAME( delta::chain::shared_string )
#endif

FC_REFLECT_ENUM( delta::chain::bandwidth_type, (post)(forum)(market) )
