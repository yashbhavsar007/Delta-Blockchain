 ...
 */
#pragma once
#include <delta/chain/block_log.hpp>
#include <delta/chain/fork_database.hpp>
#include <delta/chain/global_property_object.hpp>
#include <delta/chain/hardfork_property_object.hpp>
#include <delta/chain/node_property_object.hpp>
#include <delta/chain/notifications.hpp>

#include <delta/chain/util/advanced_benchmark_dumper.hpp>
#include <delta/chain/util/signal.hpp>
...
namespace delta { namespace chain {
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = DELTA_INIT_SUPPLY;
         ...
         chain_id_type delta_chain_id = DELTA_CHAIN_ID;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns DELTA_NULL_WITNESS
          */
         ...
         /** @return the dbd created and deposited to_account, may return DELTA if there is no median feed */
         std::pair< asset, asset > create_dbd( const account_object& to_account, asset delta, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset delta, bool to_reward_balance=false );
          ...
          * DELTA.  Return 0 DBD if there isn't a current_median_history
          */
         asset to_dbd( const asset& delta )const;
         asset to_delta( const asset& dbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = DELTA_INIT_SUPPLY );
   ...
   protected:
      ...
      private:
         ...
         fc::time_point_sec            _hardfork_times[ DELTA_NUM_HARDFORKS + 1 ];
         protocol::hardfork_version    _hardfork_versions[ DELTA_NUM_HARDFORKS + 1 ];
