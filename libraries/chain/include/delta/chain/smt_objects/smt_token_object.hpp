...
public:
   ...
   /// smt_setup_emissions
   time_point_sec       schedule_time = DELTA_GENESIS_TIME;
   delta::protocol::
   smt_emissions_unit   emissions_unit;
   uint32_t             interval_seconds = 0;
   uint32_t             interval_count = 0;
   time_point_sec       lep_time = DELTA_GENESIS_TIME;
   time_point_sec       rep_time = DELTA_GENESIS_TIME;
   asset                lep_abs_amount = asset( 0, DELTA_SYMBOL );
   asset                rep_abs_amount = asset( 0, DELTA_SYMBOL );
   ...
   // smt_cap_reveal
   share_type  delta_units_min_cap = -1;
   share_type  delta_units_hard_cap = -1;
};
...
} } // namespace delta::chain

FC_REFLECT_ENUM( delta::chain::smt_phase,
                  (account_elevated)
                  (setup_completed)
                  (contribution_begin_time_completed)
                  (contribution_end_time_completed)
...
FC_REFLECT( delta::chain::smt_token_object::smt_market_maker_state,
   (delta_balance)
   (token_balance)
   (reserve_ratio)
)

FC_REFLECT( delta::chain::smt_token_object,
   (id)
   (liquid_symbol)
   (control_account)
   (phase)
   ...
   (delta_units_min_cap)
   (delta_units_hard_cap)
)

FC_REFLECT( delta::chain::smt_event_token_object,
   (id)
   (parent)
   (phase)
   (generation_begin_time)
...
CHAINBASE_SET_INDEX_TYPE( delta::chain::smt_event_token_object, delta::chain::smt_event_token_index )
