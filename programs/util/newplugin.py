...
target_link_libraries( {plugin_provider}_{plugin_name} deltait_app deltait_chain deltait_protocol )
...
{{
   public:
      {plugin_name}_api( const delta::app::api_context& ctx );
...
class {plugin_name}_plugin : public delta::app::plugin
{{
   public:
      {plugin_name}_plugin( delta::app::application* app );
...
"{plugin_name}_api.cpp" :
"""
#include <delta/app/api_context.hpp>
#include <delta/app/application.hpp>
...
{{
   public:
      {plugin_name}_api_impl( delta::app::application& _app );

      std::shared_ptr< {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin > get_plugin();

      delta::app::application& app;
}};

{plugin_name}_api_impl::{plugin_name}_api_impl( delta::app::application& _app ) : app( _app )
{{}}
...
{plugin_name}_api::{plugin_name}_api( const delta::app::api_context& ctx )
{{
...
{{
   public:
      {plugin_name}_plugin_impl( delta::app::application& app );
      ...
      delta::app::application& _app;
      boost::signals2::scoped_connection _applied_block_conn;
}};

{plugin_name}_plugin_impl::{plugin_name}_plugin_impl( delta::app::application& app )
  : _app(app) {{}}
...
{plugin_name}_plugin::{plugin_name}_plugin( delta::app::application* app )
   : plugin(app)
{{
...
DELTA_DEFINE_PLUGIN( {plugin_name}, {plugin_provider}::plugin::{plugin_name}::{plugin_name}_plugin )
""",
}
...
def main(argv):
    ...
    parser.add_argument("provider", help="Name of plugin provider (delta for plugins developed by Deltait)")
