...
using delta::protocol::calc_percent_reward_per_block;
using delta::protocol::calc_percent_reward_per_round;
using delta::protocol::calc_percent_reward_per_hour;
...
rvec shows total number of DELTA satoshis created since genesis for:
...
- Missed blocks not modeled (lowers DELTA supply, small)
- Miner queue length very approximately modeled (assumed to go to 100 during the first blocks and then stay there) (may lower or raise DELTA supply, very small)
- Creation / destruction of DELTA used to back DBD not modeled (moves DELTA supply in direction opposite to changes in dollar value of 1 DELTA, large)
- Interest paid to DBD not modeled (raises DELTA supply, medium)
- Lost / forgotten private keys / wallets and deliberate burning of DELTA not modeled (lowers DELTA supply, unknown but likely small)
...
#define DELTA_FIRST_CASHOUT_TIME              (fc::time_point_sec(1467590400))  /// July 4th
*/
   ...
   {
      uint32_t vesting_factor = (block_num < DELTA_START_VESTING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< DELTA_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, DELTA_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;

      share_type content_reward  = calc_percent_reward_per_block< DELTA_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, DELTA_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;

      share_type producer_reward = calc_percent_reward_per_block< DELTA_PRODUCER_APR_PERCENT >( current_supply );
      reward_delta[ PRODUCER_OFF ] = std::max( producer_reward, DELTA_MIN_PRODUCER_REWARD.amount );
      ...
      if( (block_num % DELTA_MAX_WITNESSES) == 0 )
         ++pow_deficit;

      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< DELTA_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, DELTA_MIN_POW_REWARD.amount );
         if( block_num < DELTA_START_MINER_VOTING_BLOCK )
            pow_reward *= DELTA_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      if( (block_num > liquidity_begin_block) && ((block_num % DELTA_LIQUIDITY_REWARD_BLOCKS) == 0) )
      {
         liquidity_reward = calc_percent_reward_per_hour< DELTA_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, DELTA_MIN_LIQUIDITY_REWARD.amount );
      }
   ...
   for( uint32_t b=1; b<10*DELTA_BLOCKS_PER_YEAR; b++ )
   {
