...
#define DELTA_EXAMPLE_API_PLUGIN_NAME "example_api"

namespace delta { namespace example_api_plugin {
   ...
   class example_api_plugin : public appbase::plugin< example_api_plugin >
   {
      public:
         ...
         static const std::string& name() { static std::string name = DELTA_EXAMPLE_API_PLUGIN_NAME; return name; }
...
} } // delta::example_api_plugin

// Args and return types need to be reflected. hello_world_args does not because it is a typedef of a reflected type
FC_REFLECT( delta::example_api_plugin::hello_world_return, (message) )
FC_REFLECT( delta::example_api_plugin::echo_args, (call) )
FC_REFLECT( delta::example_api_plugin::echo_return, (response) )
