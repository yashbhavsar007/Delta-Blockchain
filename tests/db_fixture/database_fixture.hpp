...
extern uint32_t ( DELTA_TESTING_GENESIS_TIMESTAMP );

#define PUSH_TX \
   delta::chain::test::_push_transaction

#define PUSH_BLOCK \
   delta::chain::test::_push_block
...
/*#define DELTA_REQUIRE_THROW( expr, exc_type )          \
{                                                         \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "DELTA_REQUIRE_THROW begin "        \
         << req_throw_info << std::endl;                  \
   BOOST_REQUIRE_THROW( expr, exc_type );                 \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "DELTA_REQUIRE_THROW end "          \
         << req_throw_info << std::endl;                  \
}*/
...
#define DELTA_CHECK_THROW( expr, exc_type )            \
{                                                         \
   std::string req_throw_info = fc::json::to_string(      \
      fc::mutable_variant_object()                        \
      ("source_file", __FILE__)                           \
   ...
   if( fc::enable_record_assert_trip )                    \
      std::cout << "DELTA_CHECK_THROW begin "          \
         << req_throw_info << std::endl;                  \
   BOOST_CHECK_THROW( expr, exc_type );                   \
   if( fc::enable_record_assert_trip )                    \
      std::cout << "DELTA_CHECK_THROW end "            \
         << req_throw_info << std::endl;                  \
}
...
{ \
   const auto temp = op.field; \
   op.field = value; \
   DELTA_REQUIRE_THROW( op.validate(), exc_type ); \
   op.field = temp; \
}
...
{ \
   ...
   trx.operations.back() = op; \
   op.field = bak; \
   DELTA_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
#define ASSET( s ) \
   delta::plugins::condenser_api::legacy_asset::from_string( s ).to_asset()
...
{ \
   signed_transaction tx; \
   OP2TX(OP,tx,KEY) \
   DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), EXCEPTION ); \
}
...
struct database_fixture {
   ...
   string debug_key = delta::utilities::key_to_wif( init_account_priv_key );
...
struct t_smt_database_fixture : public T
{
   ...
   //smt_setup_operation
   smt_generation_unit get_generation_unit ( const units& delta_unit = units(), const units& token_unit = units() );
   ...
   (
      ...
      const smt_cap_commitment& min_delta_units_commitment = smt_cap_commitment(),
      const smt_cap_commitment& hard_cap_delta_units_commitment = smt_cap_commitment(),
...
{
   private:
      delta::plugins::json_rpc::json_rpc_plugin* rpc_plugin;
