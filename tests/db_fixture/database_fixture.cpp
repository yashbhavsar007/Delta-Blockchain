...
#include <delta/utilities/tempdir.hpp>
...
#include <delta/plugins/webserver/webserver_plugin.hpp>
#include <delta/plugins/witness/witness_plugin.hpp>

#include <delta/plugins/condenser_api/condenser_api_plugin.hpp>
...
uint32_t DELTA_TESTING_GENESIS_TIMESTAMP = 1431700000;

using namespace delta::plugins::webserver;
using namespace delta::plugins::database_api;
using namespace delta::plugins::block_api;
using delta::plugins::condenser_api::condenser_api_plugin;
...
clean_database_fixture::clean_database_fixture()
{
   ...
   appbase::app().register_plugin< delta::plugins::account_history::account_history_plugin >();
   db_plugin = &appbase::app().register_plugin< delta::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< delta::plugins::rc::rc_plugin >();
   appbase::app().register_plugin< delta::plugins::witness::witness_plugin >();

   db_plugin->logging = false;
   appbase::app().initialize<
      delta::plugins::account_history::account_history_plugin,
      delta::plugins::debug_node::debug_node_plugin,
      delta::plugins::rc::rc_plugin,
      delta::plugins::witness::witness_plugin
      >( argc, argv );

   delta::plugins::rc::rc_plugin_skip_flags rc_skip;
   ...
   appbase::app().get_plugin< delta::plugins::rc::rc_plugin >().set_rc_plugin_skip_flags( rc_skip );
   ...
   generate_block();
   db->set_hardfork( DELTA_BLOCKCHAIN_VERSION.minor() );
   generate_block();
...
void clean_database_fixture::validate_database()
{
   ...
   appbase::app().get_plugin< delta::plugins::rc::rc_plugin >().validate_database();
}
...
void database_fixture::open_database()
{
   if( !data_dir )
   {
      data_dir = fc::temp_directory( delta::utilities::temp_directory_path() );
...
{
   skip |= default_skip;
   db_plugin->debug_generate_blocks( delta::utilities::key_to_wif( key ), 1, skip, miss_blocks );
}
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
   ...
   BOOST_REQUIRE( ( db->head_block_time() - timestamp ).to_seconds() < DELTA_BLOCK_INTERVAL );
}
   ...
   )
{
   try
   {
      ...
      op.creator = creator;
      op.fee = asset( actual_fee, DELTA_SYMBOL );
      ...
      trx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      ...
      if( fee_remainder > 0 )
      {
         vest( DELTA_INIT_MINER_NAME, name, asset( fee_remainder, DELTA_SYMBOL ) );
      }
...
)
{
   try
   {
      return account_create(
         name,
         DELTA_INIT_MINER_NAME,
         init_account_priv_key,
         std::max( db->get_witness_schedule_object().median_props.account_creation_fee.amount * DELTA_CREATE_ACCOUNT_WITH_DELTA_MODIFIER, share_type( 100 ) ),
         key,
         post_key,
         "" );
   }
   ...
   )
{
   try
   {
      transfer( DELTA_INIT_MINER_NAME, account_name, asset( amount, DELTA_SYMBOL ) );
      ...
      db_plugin->debug_update( [=]( database& db)
      {
         ...
         if( amount.symbol == DBD_SYMBOL )
         {
            ...
            if( median_feed.current_median_history.is_null() )
               ...
               {
                  f.current_median_history = price( asset( 1, DBD_SYMBOL ), asset( 1, DELTA_SYMBOL ) );
               });
         }
   ...
   const asset& amount )
{
   try
   {
      ...
      {
         db->adjust_balance( account_name, -amount );
         db->adjust_balance( account_name, db->to_delta( amount ) );
         db->adjust_supply( -amount );
         db->adjust_supply( db->to_delta( amount ) );
      }
   } FC_CAPTURE_AND_RETHROW( (account_name)(amount) )
}
...
void database_fixture::vest( const string& from, const string& to, const asset& amount )
{
   try
   {
      FC_ASSERT( amount.symbol == DELTA_SYMBOL, "Can only vest TESTS" );
...
void database_fixture::vest( const string& from, const share_type& amount )
{
   try
   {
      ...
      op.from = from;
      op.to = "";
      op.amount = asset( amount, DELTA_SYMBOL );
...
void database_fixture::set_price_feed( const price& new_price )
{
   for( size_t i = 1; i < 8; i++ )
   {
      ...
      op.owner = DELTA_INIT_MINER_NAME + fc::to_string( i );
   ...
   generate_blocks( DELTA_BLOCKS_PER_HOUR );
...
{
   trx.clear();
   for( size_t i=0; i<DELTA_MAX_WITNESSES; i++ )
   {
      witness_set_properties_operation op;
      op.owner = DELTA_INIT_MINER_NAME + (i == 0 ? "" : fc::to_string( i ));
      op.props = props;
   ...
   for( size_t i=0; i<2*DELTA_MAX_WITNESSES+1; i++ )
   {
      generate_block();
   ...
   FC_ASSERT( false, "Couldn't apply properties in ${n} blocks", ("n", 2*DELTA_MAX_WITNESSES+1) );
}
...
vector< operation > database_fixture::get_last_operations( uint32_t num_ops )
{
   ...
   while( itr != acc_hist_idx.begin() && ops.size() < num_ops )
   {
      ...
      ops.push_back( fc::raw::unpack_from_vector< delta::chain::operation >( serialized_op ) );
   }
   ...
   uint8_t token_decimal_places )
{
   ...
   try
   {
      ...
      op.smt_creation_fee = ASSET( "1000.000 DBD" );
      op.control_account = account_name;

      tx.operations.push_back( op );
      tx.set_expiration( this->db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
...
void push_invalid_operation(const operation& invalid_op, const fc::ecc::private_key& key, database* db)
{
   ...
   DELTA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
}
...
void t_smt_database_fixture< T >::create_invalid_smt( const char* control_account_name, const fc::ecc::private_key& key )
{
   ...
   DELTA_REQUIRE_THROW( set_create_op(this->db, &op_precision, control_account_name, DELTA_ASSET_MAX_DECIMALS + 1), fc::assert_exception );
}
...
template< typename T >
smt_generation_unit t_smt_database_fixture< T >::get_generation_unit( const units& delta_unit, const units& token_unit )
{
   smt_generation_unit ret;

   ret.delta_unit = delta_unit;
...
smt_cap_commitment t_smt_database_fixture< T >::get_cap_commitment( share_type amount, uint128_t nonce )
{
   ...
   else
   {
      ...
      ret.lower_bound = SMT_MIN_HARD_CAP_DELTA_UNITS; // See smt_capped_generation_policy::validate
      ret.upper_bound = DELTA_MAX_SHARE_SUPPLY/10;    // See smt_capped_generation_policy::validate
   }
...
(
   ...
   const smt_cap_commitment& min_delta_units_commitment,
   const smt_cap_commitment& hard_cap_delta_units_commitment,
...
)
{
   ...
   ret.min_delta_units_commitment = min_delta_units_commitment;
   ret.hard_cap_delta_units_commitment = hard_cap_delta_units_commitment;
...
template smt_generation_unit t_smt_database_fixture< clean_database_fixture >::get_generation_unit( const units& delta_unit, const units& token_unit );
...
json_rpc_database_fixture::json_rpc_database_fixture()
{
   ...
   db_plugin = &appbase::app().register_plugin< delta::plugins::debug_node::debug_node_plugin >();
   appbase::app().register_plugin< delta::plugins::witness::witness_plugin >();
   rpc_plugin = &appbase::app().register_plugin< delta::plugins::json_rpc::json_rpc_plugin >();
   appbase::app().register_plugin< delta::plugins::block_api::block_api_plugin >();
   appbase::app().register_plugin< delta::plugins::database_api::database_api_plugin >();
   appbase::app().register_plugin< delta::plugins::condenser_api::condenser_api_plugin >();
   ...
   appbase::app().initialize<
      ...
      delta::plugins::block_api::block_api_plugin,
      delta::plugins::database_api::database_api_plugin,
      delta::plugins::condenser_api::condenser_api_plugin
      >( argc, argv );

   appbase::app().get_plugin< delta::plugins::condenser_api::condenser_api_plugin >().plugin_startup();
