...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_authorities )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( DELTA_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_balance( bob_account, DELTA_SYMBOL ).amount.value == bob_balance.amount.value );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( alice_symbol, DELTA_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( alice_account, alice_symbol ).amount.value == alice_smt_balance.amount.value );
      BOOST_REQUIRE( db->get_balance( alice_account, DELTA_SYMBOL ).amount.value == alice_balance.amount.value );
      validate_database();
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_validate )
{
   try
   {
      ...
      op.reward_tokens.push_back( ASSET( "0.000 DBD" ) );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.reward_tokens.clear();
      ...
      op.reward_tokens.push_back( ASSET( "1.000 DBD" ) );
      op.validate();
      op.reward_tokens.clear();
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_delta + ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_delta_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == alice_dbd + ASSET( "0.000 DBD" ) );
      ...
      // Legacy symbols
      asset full_delta = ASSET( "10.000 TESTS" );
      asset full_dbd = ASSET( "10.000 DBD" );
      op.reward_tokens.push_back( full_dbd );
      op.reward_tokens.push_back( full_delta );
      op.reward_tokens.push_back( partial_vests );
      PUSH_OP(op, alice_private_key);
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_delta + full_delta );
