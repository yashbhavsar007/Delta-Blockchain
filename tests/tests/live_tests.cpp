...
BOOST_AUTO_TEST_CASE( vests_stock_split )
{
   try
   {
      ...
      auto old_vesting_fund = db->get_dynamic_global_properties().total_vesting_fund_delta;
      ...
      auto old_reward_fund = db->get_dynamic_global_properties().total_reward_fund_delta;
      ...
      while( com_itr != com_idx.end() )
      {
         ...
         if( com_itr->net_rshares.value > 0 )
         {
            ...
            u256 rf( gpo.total_reward_fund_delta.amount.value );
            auto rs2 = rs * rs;
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_vesting_fund_delta == old_vesting_fund );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_delta == old_reward_fund );
