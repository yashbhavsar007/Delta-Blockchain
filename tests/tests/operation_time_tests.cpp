...
#include <delta/plugins/rc/rc_objects.hpp>
...
using namespace delta::chain::util;
...
BOOST_AUTO_TEST_CASE( comment_payout_equalize )
{
   try
   {
      ...
      authors.emplace_back( "bob"  , bob_private_key, ASSET( "0.000 DBD" ) );
      ...
      // set a ridiculously high DELTA price ($1 / satoshi) to disable dust threshold
      set_price_feed( price( ASSET( "1.000 DBD" ), ASSET( "0.001 TESTS" ) ) );
      ...
      for( const auto& author : authors )
      {
         ...
         com.permlink = "mypost";
         com.parent_author = DELTA_ROOT_POST_PARENT;
      ...
      //auto reward_delta = db->get_dynamic_global_properties().total_reward_fund_delta;

      // generate a few blocks to seed the reward fund
      generate_blocks(10);
      //const auto& rf = db->get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME );
      //idump( (rf) );
      ...
      for( const auto& author : authors )
      {
         ...
         ilog( "${n} : ${delta} ${dbd}", ("n", author.name)("delta", a.reward_delta_balance)("dbd", a.reward_dbd_balance) );
      }
      for( const auto& voter : voters )
      {
         const account_object& a = db->get_account(voter.name);
         ilog( "${n} : ${delta} ${dbd}", ("n", voter.name)("delta", a.reward_delta_balance)("dbd", a.reward_dbd_balance) );
      }
      */
      ...
      BOOST_CHECK( alice_account.reward_dbd_balance == ASSET( "10720.000 DBD" ) );
      BOOST_CHECK( bob_account.reward_dbd_balance == ASSET( "0.000 DBD" ) );
...
BOOST_AUTO_TEST_CASE( comment_payout_dust )
{
   try
   {
      ...
      vest( DELTA_INIT_MINER_NAME, "alice", ASSET( "10.000 TESTS" ) );
      vest( DELTA_INIT_MINER_NAME, "bob", ASSET( "10.000 TESTS" ) );
      ...
      vote.author = "alice";
      vote.permlink = "test";
      vote.weight = 81 * DELTA_1_PERCENT;
      ...
      vote.voter = "bob";
      vote.author = "bob";
      vote.weight = 59 * DELTA_1_PERCENT;
      ...
      // If comments are paid out independent of order, then the last satoshi of DELTA cannot be divided among them
      const auto rf = db->get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( reward_funds )
{
   try
   {
      ...
      set_price_feed( price( ASSET( "1.000 TESTS" ), ASSET( "1.000 DBD" ) ) );
      generate_block();
      ...
      {
         const auto& post_rf = db->get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME );
         const auto& comment_rf = db->get< reward_fund_object, by_name >( DELTA_COMMENT_REWARD_FUND_NAME );
...
BOOST_AUTO_TEST_CASE( recent_claims_decay )
{
   try
   {
      ...
      auto alice_vshares = util::evaluate_reward_curve( db->get_comment( "alice", string( "test" ) ).net_rshares.value,
         db->get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME ).author_reward_curve,
         db->get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME ).content_constant );
      ...
      {
         alice_vshares -= ( alice_vshares * DELTA_BLOCK_INTERVAL ) / DELTA_RECENT_RSHARES_DECAY_TIME_HF19.to_seconds();
         const auto& post_rf = db->get< reward_fund_object, by_name >( DELTA_POST_REWARD_FUND_NAME );
...
/*BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      price exchange_rate( ASSET( "1.000 TESTS" ), ASSET( "1.000 DBD" ) );
      ...
      //generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - DELTA_BLOCK_INTERVAL, true );

      auto reward_delta = db->get_dynamic_global_properties().total_reward_fund_delta + ASSET( "1.667 TESTS" );
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_delta.amount.value ) / total_rshares2 ).to_uint64(), DELTA_SYMBOL );
      auto bob_comment_discussion_rewards = asset( bob_comment_payout.amount / 4, DELTA_SYMBOL );
      bob_comment_payout -= bob_comment_discussion_rewards;
      auto bob_comment_dbd_reward = db->to_dbd( asset( bob_comment_payout.amount / 2, DELTA_SYMBOL ) );
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, DELTA_SYMBOL) ) * db->get_dynamic_global_properties().get_vesting_share_price();

      BOOST_TEST_MESSAGE( "Cause first payout" );

      generate_block();

      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_delta == reward_delta - bob_comment_payout );
...
BOOST_AUTO_TEST_CASE( comment_payout )
{
   try
   {
      ...
      generate_blocks( fc::time_point_sec( db->head_block_time().sec_since_epoch() + DELTA_CASHOUT_WINDOW_SECONDS / 2 ), true );
      ...
      generate_blocks( db->get_comment( "bob", string( "test" ) ).cashout_time - ( DELTA_BLOCK_INTERVAL / 2 ), true );
      ...
      auto reward_delta = db->get_dynamic_global_properties().total_reward_fund_delta + ASSET( "2.000 TESTS" );
      auto total_rshares2 = db->get_dynamic_global_properties().total_reward_shares2;
      ...
      auto bob_comment_payout = asset( ( ( uint128_t( bob_comment_rshares.value ) * bob_comment_rshares.value * reward_delta.amount.value ) / total_rshares2 ).to_uint64(), DELTA_SYMBOL );
      auto bob_comment_vote_rewards = asset( bob_comment_payout.amount / 2, DELTA_SYMBOL );
      bob_comment_payout -= bob_comment_vote_rewards;
      auto bob_comment_dbd_reward = asset( bob_comment_payout.amount / 2, DELTA_SYMBOL ) * exchange_rate;
      auto bob_comment_vesting_reward = ( bob_comment_payout - asset( bob_comment_payout.amount / 2, DELTA_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      auto unclaimed_payments = bob_comment_vote_rewards;
      auto alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), DELTA_SYMBOL );
      auto alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), DELTA_SYMBOL );
      auto bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards.amount.value ) / bob_comment_vote_total ), DELTA_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_dynamic_global_properties().total_reward_fund_delta.amount.value == reward_delta.amount.value - ( bob_comment_payout + bob_comment_vote_rewards - unclaimed_payments ).amount.value );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - ( DELTA_BLOCK_INTERVAL / 2 ), true );
      ...
      reward_delta = db->get_dynamic_global_properties().total_reward_fund_delta + ASSET( "2.000 TESTS" );
      ...
      u256 rf( reward_delta.amount.value );
      u256 trs2 = total_rshares2.hi;
      trs2 = ( trs2 << 64 ) + total_rshares2.lo;
      auto rs2 = rs*rs;

      auto alice_comment_payout = asset( static_cast< uint64_t >( ( rf * rs2 ) / trs2 ), DELTA_SYMBOL );
      auto alice_comment_vote_rewards = asset( alice_comment_payout.amount / 2, DELTA_SYMBOL );
      alice_comment_payout -= alice_comment_vote_rewards;
      auto alice_comment_dbd_reward = asset( alice_comment_payout.amount / 2, DELTA_SYMBOL ) * exchange_rate;
      auto alice_comment_vesting_reward = ( alice_comment_payout - asset( alice_comment_payout.amount / 2, DELTA_SYMBOL ) ) * db->get_dynamic_global_properties().get_vesting_share_price();
      unclaimed_payments = alice_comment_vote_rewards;
      alice_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), DELTA_SYMBOL );
      alice_vote_vesting = alice_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      bob_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), DELTA_SYMBOL );
      bob_vote_vesting = bob_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      sam_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), DELTA_SYMBOL );
      sam_vote_vesting = sam_vote_reward * db->get_dynamic_global_properties().get_vesting_share_price();
      auto dave_vote_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "dave" ) ).id ) )->weight ) * alice_comment_vote_rewards.amount.value ) / alice_comment_vote_total ), DELTA_SYMBOL );
      ...
      BOOST_REQUIRE( ( db->get_dynamic_global_properties().total_reward_fund_delta + alice_comment_payout + alice_comment_vote_rewards - unclaimed_payments ).amount.value == reward_delta.amount.value );
...
BOOST_AUTO_TEST_CASE( nested_comments )
{
   try
   {
      ...
      vote_op.weight = DELTA_100_PERCENT;
      ...
      tx.operations.push_back( vote_op );
      vote_op.author = "dave";
      vote_op.weight = DELTA_1_PERCENT * 20;
      tx.operations.push_back( vote_op );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - fc::seconds( DELTA_BLOCK_INTERVAL ), true );

      auto gpo = db->get_dynamic_global_properties();
      uint128_t reward_delta = gpo.total_reward_fund_delta.amount.value + ASSET( "2.000 TESTS" ).amount.value;
      ...
      auto alice_comment_reward = ( ( reward_delta * alice_comment.net_rshares.value * alice_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( alice_comment.net_rshares.value ) * ( alice_comment.net_rshares.value );
      reward_delta -= alice_comment_reward;
      auto alice_comment_vote_rewards = alice_comment_reward / 2;
      alice_comment_reward -= alice_comment_vote_rewards;

      auto alice_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), DELTA_SYMBOL );
      auto bob_vote_alice_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "alice", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * alice_comment_vote_rewards ) / alice_comment.total_vote_weight ), DELTA_SYMBOL );
      reward_delta += alice_comment_vote_rewards - ( alice_vote_alice_reward + bob_vote_alice_reward ).amount.value;

      auto bob_comment_reward = ( ( reward_delta * bob_comment.net_rshares.value * bob_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( bob_comment.net_rshares.value ) * bob_comment.net_rshares.value;
      reward_delta -= bob_comment_reward;
      auto bob_comment_vote_rewards = bob_comment_reward / 2;
      bob_comment_reward -= bob_comment_vote_rewards;

      auto alice_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "alice" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), DELTA_SYMBOL );
      auto bob_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), DELTA_SYMBOL );
      auto sam_vote_bob_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "bob", string( "test" ).id, db->get_account( "sam" ) ).id ) )->weight ) * bob_comment_vote_rewards ) / bob_comment.total_vote_weight ), DELTA_SYMBOL );
      reward_delta += bob_comment_vote_rewards - ( alice_vote_bob_reward + bob_vote_bob_reward + sam_vote_bob_reward ).amount.value;

      auto dave_comment_reward = ( ( reward_delta * dave_comment.net_rshares.value * dave_comment.net_rshares.value ) / total_rshares2 ).to_uint64();
      total_rshares2 -= uint128_t( dave_comment.net_rshares.value ) * dave_comment.net_rshares.value;
      reward_delta -= dave_comment_reward;
      auto dave_comment_vote_rewards = dave_comment_reward / 2;
      dave_comment_reward -= dave_comment_vote_rewards;

      auto bob_vote_dave_reward = asset( static_cast< uint64_t >( ( u256( vote_idx.find( std::make_tuple( db->get_comment( "dave", string( "test" ).id, db->get_account( "bob" ) ).id ) )->weight ) * dave_comment_vote_rewards ) / dave_comment.total_vote_weight ), DELTA_SYMBOL );
      reward_delta += dave_comment_vote_rewards - bob_vote_dave_reward.amount.value;
      ...
      auto alice_comment_total_payout = db->to_dbd( asset( alice_pays_alice_dbd + alice_pays_alice_vest, DELTA_SYMBOL ) );
      alice_comment_total_payout += db->to_dbd( asset( bob_pays_alice_dbd + bob_pays_alice_vest, DELTA_SYMBOL ) );
      alice_comment_total_payout += db->to_dbd( asset( dave_pays_alice_dbd + dave_pays_alice_vest, DELTA_SYMBOL ) );
      auto bob_comment_total_payout = db->to_dbd( asset( bob_pays_bob_dbd + bob_pays_bob_vest, DELTA_SYMBOL ) );
      bob_comment_total_payout += db->to_dbd( asset( dave_pays_bob_dbd + dave_pays_bob_vest, DELTA_SYMBOL ) );
      auto sam_comment_total_payout = db->to_dbd( asset( dave_pays_sam_dbd + dave_pays_sam_vest, DELTA_SYMBOL ) );
      auto dave_comment_total_payout = db->to_dbd( asset( dave_pays_dave_dbd + dave_pays_dave_vest, DELTA_SYMBOL ) );
      ...
      auto alice_total_dbd = alice_starting_dbd + asset( alice_pays_alice_dbd + bob_pays_alice_dbd + dave_pays_alice_dbd, DELTA_SYMBOL ) * exchange_rate;
      auto alice_total_vesting = alice_starting_vesting + asset( alice_pays_alice_vest + bob_pays_alice_vest + dave_pays_alice_vest + alice_vote_alice_reward.amount + alice_vote_bob_reward.amount, DELTA_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto bob_total_dbd = bob_starting_dbd + asset( bob_pays_bob_dbd + dave_pays_bob_dbd, DELTA_SYMBOL ) * exchange_rate;
      auto bob_total_vesting = bob_starting_vesting + asset( bob_pays_bob_vest + dave_pays_bob_vest + bob_vote_alice_reward.amount + bob_vote_bob_reward.amount + bob_vote_dave_reward.amount, DELTA_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto sam_total_dbd = sam_starting_dbd + asset( dave_pays_sam_dbd, DELTA_SYMBOL ) * exchange_rate;
      auto sam_total_vesting = bob_starting_vesting + asset( dave_pays_sam_vest + sam_vote_bob_reward.amount, DELTA_SYMBOL ) * gpo.get_vesting_share_price();
      ...
      auto dave_total_dbd = dave_starting_dbd + asset( dave_pays_dave_dbd, DELTA_SYMBOL ) * exchange_rate;
      auto dave_total_vesting = dave_starting_vesting + asset( dave_pays_dave_vest, DELTA_SYMBOL ) * gpo.get_vesting_share_price();
...
BOOST_AUTO_TEST_CASE( vesting_withdrawals )
{
   try
   {
      ...
      auto next_withdrawal = db->head_block_time() + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS;
      ...
      generate_blocks( next_withdrawal - ( DELTA_BLOCK_INTERVAL / 2 ), true);
      ...
      for( int i = 1; i < DELTA_VESTING_WITHDRAW_INTERVALS - 1; i++ )
      {
         generate_blocks( db->head_block_time() + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS );
         ...
         if ( i == DELTA_VESTING_WITHDRAW_INTERVALS - 1 )
            BOOST_REQUIRE( alice.next_vesting_withdrawal == fc::time_point_sec::maximum() );
         else
            BOOST_REQUIRE( alice.next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
      ...
      if (  to_withdraw.amount.value % withdraw_rate.amount.value != 0 )
      {
         ...
         generate_blocks( db->head_block_time() + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS, true );
         fill_op = get_last_operations( 1 )[0].get< fill_vesting_withdraw_operation >();
         gpo = db->get_dynamic_global_properties();

         BOOST_REQUIRE( db->get_account( "alice" ).next_vesting_withdrawal.sec_since_epoch() == ( old_next_vesting + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS ).sec_since_epoch() );
         BOOST_REQUIRE( fill_op.from_account == "alice" );
         BOOST_REQUIRE( fill_op.to_account == "alice" );
         BOOST_REQUIRE( fill_op.withdrawn.amount.value == withdraw_rate.amount.value );
         BOOST_REQUIRE( std::abs( ( fill_op.deposited - fill_op.withdrawn * gpo.get_vesting_share_price() ).amount.value ) <= 1 );
...
BOOST_AUTO_TEST_CASE( vesting_withdraw_route )
{
   try
   {
      ...
      op.to_account = "bob";
      op.percent = DELTA_1_PERCENT * 50;
      op.auto_vest = true;
      tx.operations.push_back( op );

      BOOST_TEST_MESSAGE( "Setting up sam destination" );
      op.to_account = "sam";
      op.percent = DELTA_1_PERCENT * 30;
      op.auto_vest = false;
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...

      {
         ...
         BOOST_REQUIRE( alice.balance == old_alice_balance + asset( ( vesting_withdraw_rate.amount * DELTA_1_PERCENT * 20 ) / DELTA_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
         BOOST_REQUIRE( bob.vesting_shares == old_bob_vesting + asset( ( vesting_withdraw_rate.amount * DELTA_1_PERCENT * 50 ) / DELTA_100_PERCENT, VESTS_SYMBOL ) );
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * DELTA_1_PERCENT * 30 ) / DELTA_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      ...
      op.to_account = "sam";
      op.percent = DELTA_1_PERCENT * 50 + 1;
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      ...
      generate_blocks( db->get_account( "alice" ).next_vesting_withdrawal, true );
      {
         ...
         BOOST_REQUIRE( bob.balance == old_bob_balance );
         BOOST_REQUIRE( sam.vesting_shares == old_sam_vesting );
         BOOST_REQUIRE( sam.balance ==  old_sam_balance + asset( ( vesting_withdraw_rate.amount * DELTA_1_PERCENT * 50 ) / DELTA_100_PERCENT, VESTS_SYMBOL ) * db->get_dynamic_global_properties().get_vesting_share_price() );
      }
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_mean )
{
   try
   {
      ...
      generate_blocks( 30 / DELTA_BLOCK_INTERVAL );
      ...
      for( int i = 0; i < 7; i++ )
      {
         transfer( DELTA_INIT_MINER_NAME, accounts[i], asset( 10000, DELTA_SYMBOL ) );
      ...
      ops[0].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset( 100000, DELTA_SYMBOL ) );
      ops[1].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset( 105000, DELTA_SYMBOL ) );
      ops[2].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset(  98000, DELTA_SYMBOL ) );
      ops[3].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset(  97000, DELTA_SYMBOL ) );
      ops[4].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset(  99000, DELTA_SYMBOL ) );
      ops[5].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset(  97500, DELTA_SYMBOL ) );
      ops[6].exchange_rate = price( asset( 1000, DBD_SYMBOL ), asset( 102000, DELTA_SYMBOL ) );

      for( int i = 0; i < 7; i++ )
      {
         txs[i].set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( DELTA_BLOCKS_PER_HOUR ); // Jump forward 1 hour
      ...
      BOOST_REQUIRE( feed_history.current_median_history == price( asset( 1000, DBD_SYMBOL ), asset( 99000, DELTA_SYMBOL) ) );
      BOOST_REQUIRE( feed_history.price_history[ 0 ] == price( asset( 1000, DBD_SYMBOL ), asset( 99000, DELTA_SYMBOL) ) );
      validate_database();
      ...
      for ( int i = 0; i < 23; i++ )
      {
         ...
         for( int j = 0; j < 7; j++ )
         {
            ...
            ops[j].exchange_rate = price( ops[j].exchange_rate.base, asset( ops[j].exchange_rate.quote.amount + 10, DELTA_SYMBOL ) );
            txs[j].set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( convert_delay )
{
   try
   {
      ...
      generate_block();
      vest( DELTA_INIT_MINER_NAME, "alice", ASSET( "10.000 TESTS" ) );
      fund( "alice", ASSET( "25.000 DBD" ) );

      set_price_feed( price( ASSET( "1.000 DBD" ), ASSET( "1.250 TESTS" ) ) );

      convert_operation op;
      signed_transaction tx;

      auto start_balance = ASSET( "25.000 DBD" );
      ...
      generate_blocks( db->head_block_time() + DELTA_CONVERSION_DELAY - fc::seconds( DELTA_BLOCK_INTERVAL / 2 ), true );
      ...
      BOOST_REQUIRE( vop.amount_in.amount.value == ASSET( "2.000 DBD" ).amount.value );
...
BOOST_AUTO_TEST_CASE( delta_inflation )
{
   try
   {
   /*
      BOOST_TEST_MESSAGE( "Testing DELTA Inflation until the vesting start block" );
      ...
      auto new_rewards = std::max( DELTA_MIN_CONTENT_REWARD, asset( ( DELTA_CONTENT_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) )
         + std::max( DELTA_MIN_CURATE_REWARD, asset( ( DELTA_CURATE_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) );
      auto witness_pay = std::max( DELTA_MIN_PRODUCER_REWARD, asset( ( DELTA_PRODUCER_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) );
      auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
      auto new_vesting_delta = asset( 0, DELTA_SYMBOL );
      auto new_vesting_shares = gpo.total_vesting_shares;

      if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
      {
         new_vesting_delta += witness_pay;
         new_vesting_shares += witness_pay * ( gpo.total_vesting_shares / gpo.total_vesting_fund_delta );
      }

      auto new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_delta;
      new_rewards += gpo.total_reward_fund_delta;
      new_vesting_delta += gpo.total_vesting_fund_delta;
      ...
      BOOST_REQUIRE( gpo.total_reward_fund_delta.amount.value == new_rewards.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_fund_delta.amount.value == new_vesting_delta.amount.value );
      ...
      while( db->head_block_num() < DELTA_START_VESTING_BLOCK - 1)
      {
         ...
         new_rewards = std::max( DELTA_MIN_CONTENT_REWARD, asset( ( DELTA_CONTENT_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) )
            + std::max( DELTA_MIN_CURATE_REWARD, asset( ( DELTA_CURATE_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) );
         witness_pay = std::max( DELTA_MIN_PRODUCER_REWARD, asset( ( DELTA_PRODUCER_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) );
         new_vesting_delta = asset( 0, DELTA_SYMBOL );
         ...
         if ( db->get_account( witness_name ).vesting_shares.amount.value == 0 )
         {
            ...
            witness_pay = asset( 0, DELTA_SYMBOL );
         }

         new_supply = gpo.current_supply + new_rewards + witness_pay + new_vesting_delta;
         new_rewards += gpo.total_reward_fund_delta;
         new_vesting_delta += gpo.total_vesting_fund_delta;
      ...
      while( db->head_block_num() < DELTA_START_MINER_VOTING_BLOCK - 1 )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( DELTA_MIN_PRODUCER_REWARD, asset( ( DELTA_PRODUCER_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) );
         auto witness_pay_shares = asset( 0, VESTS_SYMBOL );
         new_vesting_delta = asset( ( witness_pay + new_rewards ).amount * 9, DELTA_SYMBOL );
         new_vesting_shares = gpo.total_vesting_shares;
      ...
      for( int i = 0; i < DELTA_BLOCKS_PER_DAY; i++ )
      {
         virtual_supply = gpo.virtual_supply;
         witness_name = db->get_scheduled_witness(1);
         old_witness_balance = db->get_account( witness_name ).balance;
         ...
         witness_pay = std::max( DELTA_MIN_PRODUCER_REWARD, asset( ( DELTA_PRODUCER_APR * gpo.virtual_supply.amount ) / ( DELTA_BLOCKS_PER_YEAR * 100 ), DELTA_SYMBOL ) );
         witness_pay_shares = witness_pay * gpo.get_vesting_share_price();
         new_vesting_delta = asset( ( witness_pay + new_rewards ).amount * 9, DELTA_SYMBOL ) + witness_pay;
         new_vesting_shares = gpo.total_vesting_shares + witness_pay_shares;
         new_supply = gpo.current_supply + new_rewards + new_vesting_delta;
         new_rewards += gpo.total_reward_fund_delta;
         new_vesting_delta += gpo.total_vesting_fund_delta;
      ...
      vesting_delta = gpo.total_vesting_fund_delta;
      reward_delta = gpo.total_reward_fund_delta;
      ...
      BOOST_REQUIRE_EQUAL( gpo.total_vesting_fund_delta.amount.value,
         ( vesting_delta.amount.value
            + ( ( ( uint128_t( virtual_supply.amount.value ) / 10 ) / DELTA_BLOCKS_PER_YEAR ) * 9 )
            + ( uint128_t( virtual_supply.amount.value ) / 100 / DELTA_BLOCKS_PER_YEAR ) ).to_uint64() );
      BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_delta.amount.value,
         reward_delta.amount.value + virtual_supply.amount.value / 10 / DELTA_BLOCKS_PER_YEAR + virtual_supply.amount.value / 10 / DELTA_BLOCKS_PER_DAY );
      BOOST_REQUIRE_EQUAL( db->get_account( witness_name ).vesting_shares.amount.value,
         old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / DELTA_BLOCKS_PER_YEAR ) * DELTA_1_PERCENT ) / DELTA_100_PERCENT, DELTA_SYMBOL ) * ( vesting_shares / vesting_delta ) ).amount.value );
      validate_database();
      */
   }
   FC_LOG_AND_RETHROW();
...
BOOST_AUTO_TEST_CASE( dbd_interest )
{
   try
   {
      ...
      fund( "alice", ASSET( "31.903 DBD" ) );

      auto start_time = db->get_account( "alice" ).dbd_seconds_last_update;
      auto alice_dbd = db->get_account( "alice" ).dbd_balance;

      generate_blocks( db->head_block_time() + fc::seconds( DELTA_DBD_INTEREST_COMPOUND_INTERVAL_SEC ), true );

      transfer_operation transfer;
      transfer.to = "bob";
      transfer.from = "alice";
      transfer.amount = ASSET( "1.000 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      ...
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).dbd_balance.amount.value) == alice_dbd.amount.value - ASSET( "1.000 DBD" ).amount.value + ( ( ( ( uint128_t( alice_dbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() ) / DELTA_SECONDS_PER_YEAR ) * gpo.dbd_interest_rate ) / DELTA_100_PERCENT ).to_uint64() );
      BOOST_REQUIRE( interest_op.owner == "alice" );
      BOOST_REQUIRE( interest_op.interest.amount.value == db->get_account( "alice" ).dbd_balance.amount.value - ( alice_dbd.amount.value - ASSET( "1.000 DBD" ).amount.value ) );
      validate_database();
      ...
      generate_blocks( db->head_block_time() + fc::seconds( DELTA_DBD_INTEREST_COMPOUND_INTERVAL_SEC / 2 ), true );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance.amount.value == alice_dbd.amount.value - ASSET( "1.000 DBD" ).amount.value );
      validate_database();
      ...
      generate_blocks( db->head_block_time() + fc::seconds( ( DELTA_DBD_INTEREST_COMPOUND_INTERVAL_SEC * 7 ) / 3 ), true );
      ...
      BOOST_REQUIRE( static_cast<uint64_t>(db->get_account( "alice" ).dbd_balance.amount.value) == alice_dbd.amount.value - ASSET( "1.000 DBD" ).amount.value + ( ( ( ( uint128_t( alice_dbd.amount.value ) * ( db->head_block_time() - start_time ).to_seconds() + alice_coindays ) / DELTA_SECONDS_PER_YEAR ) * gpo.dbd_interest_rate ) / DELTA_100_PERCENT ).to_uint64() );
      validate_database();
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( liquidity_rewards )
{
   ...
   try
   {
      ...
      vest( DELTA_INIT_MINER_NAME, "alice", ASSET( "10.000 TESTS" ) );
      vest( DELTA_INIT_MINER_NAME, "bob", ASSET( "10.000 TESTS" ) );
      vest( DELTA_INIT_MINER_NAME, "sam", ASSET( "10.000 TESTS" ) );
      vest( DELTA_INIT_MINER_NAME, "dave", ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "Rewarding Bob with TESTS" );

      auto exchange_rate = price( ASSET( "1.000 DBD" ), ASSET( "1.250 TESTS" ) );
      set_price_feed( exchange_rate );

      signed_transaction tx;

      fund( "alice", ASSET( "25.522 DBD" ) );
      ...
      generate_blocks( db->head_block_time() + DELTA_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10, true );
      ...
      alice_delta_volume += ( asset( alice_dbd.amount / 20, DBD_SYMBOL ) * exchange_rate ).amount.value;
      alice_reward_last_update = db->head_block_time();
      bob_delta_volume -= ( asset( alice_dbd.amount / 20, DBD_SYMBOL ) * exchange_rate ).amount.value;
      ...
      BOOST_REQUIRE( reward->delta_volume == alice_delta_volume );
      ...
      BOOST_REQUIRE( reward->delta_volume == bob_delta_volume );
      ...
      op.owner = "sam";
      op.amount_to_sell = asset( ( alice_dbd.amount.value / 20 ), DELTA_SYMBOL );
      ...
      op.owner = "bob";
      op.orderid = 4;
      op.amount_to_sell = asset( ( alice_dbd.amount.value / 10 ) * 3 - alice_dbd.amount.value / 20, DELTA_SYMBOL );
      ...
      op.min_to_receive = asset( ( alice_dbd.amount.value / 10 ) * 3, DELTA_SYMBOL );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( ( alice_dbd.amount.value / 10 ) * 3 - alice_dbd.amount.value / 20, DELTA_SYMBOL ).amount.value );
      ...
      BOOST_REQUIRE( fill_order_op.open_pays.amount.value == asset( alice_dbd.amount.value / 20, DELTA_SYMBOL ).amount.value );
      BOOST_REQUIRE( fill_order_op.current_owner == "alice" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 5 );
      ...
      BOOST_REQUIRE( reward->delta_volume == sam_delta_volume );
      ...
      op.min_to_receive = asset( alice_dbd.amount.value / 20 * 2, DELTA_SYMBOL );
      ...
      generate_blocks( db->head_block_time() + fc::seconds( DELTA_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10.to_seconds() / 2 ), true );
      ...
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == asset( alice_dbd.amount.value / 20, DELTA_SYMBOL ).amount.value );
      ...
      alice_delta_volume += alice_dbd.amount.value / 20;
      alice_reward_last_update = db->head_block_time();
      sam_delta_volume -= alice_dbd.amount.value / 20;
      sam_reward_last_update = db->head_block_time();
      ...
      op.owner = "alice";
      op.amount_to_sell = asset( 8 * ( alice_dbd.amount.value / 20 ), DELTA_SYMBOL );
      ...
      op.min_to_receive = asset( op.amount_to_sell.amount, DELTA_SYMBOL );
      op.orderid = 10;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      BOOST_REQUIRE( reward->delta_volume == dave_delta_volume );
      ...
      bob_delta_volume += op.amount_to_sell.amount.value;
      bob_reward_last_update = db->head_block_time();
      dave_delta_volume -= op.amount_to_sell.amount.value;
      dave_reward_last_update = db->head_block_time();
      ...
      generate_blocks( DELTA_LIQUIDITY_REWARD_BLOCKS - ( db->head_block_num() % DELTA_LIQUIDITY_REWARD_BLOCKS ) - 1 );

      BOOST_REQUIRE( db->head_block_num() % DELTA_LIQUIDITY_REWARD_BLOCKS == DELTA_LIQUIDITY_REWARD_BLOCKS - 1 );
      ...
      //alice_balance += DELTA_MIN_LIQUIDITY_REWARD;
      ...
      DELTA_REQUIRE_THROW( ops[0].get< liquidity_reward_operation>(), fc::exception );
      //BOOST_REQUIRE( ops[0].get< liquidity_reward_operation>().payout.amount.value == DELTA_MIN_LIQUIDITY_REWARD.amount.value );

      generate_blocks( DELTA_LIQUIDITY_REWARD_BLOCKS );

      //bob_balance += DELTA_MIN_LIQUIDITY_REWARD;
      ...
      generate_blocks( sam_reward_last_update + DELTA_LIQUIDITY_TIMEOUT_SEC - fc::seconds( DELTA_BLOCK_INTERVAL / 2 ) - DELTA_MIN_LIQUIDITY_REWARD_PERIOD_SEC , true );

      op.owner = "sam";
      op.orderid = 14;
      op.amount_to_sell = ASSET( "1.000 TESTS" );
      op.min_to_receive = ASSET( "1.000 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      ...
      generate_blocks( db->head_block_time() + ( DELTA_BLOCK_INTERVAL / 2 ) + DELTA_LIQUIDITY_TIMEOUT_SEC, true );
      ...
      op.min_to_receive.symbol = DELTA_SYMBOL;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      ...
      sam_dbd_volume = ASSET( "1.000 DBD" ).amount.value;
      sam_delta_volume = 0;
      sam_reward_last_update = db->head_block_time();
...
BOOST_AUTO_TEST_CASE( post_rate_limit )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test1" ) ).reward_weight == DELTA_100_PERCENT );

      tx.operations.clear();
      tx.signatures.clear();

      generate_blocks( db->head_block_time() + DELTA_MIN_ROOT_COMMENT_INTERVAL + fc::seconds( DELTA_BLOCK_INTERVAL ), true );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test2" ) ).reward_weight == DELTA_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test3" ) ).reward_weight == DELTA_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test4" ) ).reward_weight == DELTA_100_PERCENT );
      ...
      BOOST_REQUIRE( db->get_comment( "alice", string( "test5" ) ).reward_weight == DELTA_100_PERCENT );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_freeze )
{
   try
   {
      ...
      vote.voter = "bob";
      vote.weight = DELTA_100_PERCENT * -1;
...
BOOST_AUTO_TEST_CASE( dbd_stability )
{
   try
   {
      ...
      auto exchange_rate = price( ASSET( "1.000 DBD" ), ASSET( "10.000 TESTS" ) );
      set_price_feed( exchange_rate );

      BOOST_REQUIRE( db->get_dynamic_global_properties().dbd_print_rate == DELTA_100_PERCENT );
      ...
      db_plugin->debug_generate_blocks_until( debug_key, fc::time_point_sec( db->get_comment( comment.author, comment.permlink ).cashout_time.sec_since_epoch() - 2 * DELTA_BLOCK_INTERVAL ), true, database::skip_witness_signature );

      auto& gpo = db->get_dynamic_global_properties();

      BOOST_TEST_MESSAGE( "Changing sam and gpo to set up market cap conditions" );

      asset dbd_balance = asset( ( gpo.virtual_supply.amount * ( gpo.dbd_stop_percent + 112 ) ) / DELTA_100_PERCENT, DELTA_SYMBOL ) * exchange_rate;
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_dbd_supply = dbd_balance;
      ...
      auto comment_reward = ( gpo.total_reward_fund_delta.amount + 2000 ) - ( ( gpo.total_reward_fund_delta.amount + 2000 ) * 25 * DELTA_1_PERCENT ) / DELTA_100_PERCENT ;
      comment_reward /= 2;
      auto dbd_reward = ( comment_reward * gpo.dbd_print_rate ) / DELTA_100_PERCENT;
      auto alice_dbd = db->get_account( "alice" ).dbd_balance + db->get_account( "alice" ).reward_dbd_balance + asset( dbd_reward, DELTA_SYMBOL ) * exchange_rate;
      auto alice_delta = db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_delta_balance ;

      BOOST_TEST_MESSAGE( "Checking printing DBD has slowed" );
      BOOST_REQUIRE( db->get_dynamic_global_properties().dbd_print_rate < DELTA_100_PERCENT );

      BOOST_TEST_MESSAGE( "Pay out comment and check rewards are paid as DELTA" );
      db_plugin->debug_generate_blocks( debug_key, 1, database::skip_witness_signature );

      validate_database();

      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance + db->get_account( "alice" ).reward_dbd_balance == alice_dbd );
      BOOST_REQUIRE( db->get_account( "alice" ).balance + db->get_account( "alice" ).reward_delta_balance > alice_delta );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( "sam" ), [&]( account_object& a )
         {
            a.dbd_balance = asset( ( ( gpo.dbd_start_percent - 6 ) * dbd_balance.amount ) / gpo.dbd_stop_percent, DBD_SYMBOL );
         });
      }, database::skip_witness_signature );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.current_dbd_supply = alice_dbd + asset( ( ( gpo.dbd_start_percent - 6 ) * dbd_balance.amount ) / gpo.dbd_stop_percent, DBD_SYMBOL );
         });
      }, database::skip_witness_signature );
      ...
      while( ( db->get_dynamic_global_properties().current_dbd_supply * exchange_rate ).amount >= ( db->get_dynamic_global_properties().virtual_supply.amount * db->get_dynamic_global_properties().dbd_start_percent ) / DELTA_100_PERCENT )
      {
         auto& gpo = db->get_dynamic_global_properties();
...
BOOST_AUTO_TEST_CASE( dbd_price_feed_limit )
{
   try
   {
      ...
      price exchange_rate( ASSET( "1.000 DBD" ), ASSET( "1.000 TESTS" ) );
      set_price_feed( exchange_rate );
      ...
      auto new_exchange_rate = price( gpo.current_dbd_supply, asset( ( DELTA_100_PERCENT ) * gpo.current_supply.amount, DELTA_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( clear_null_account )
{
   try
   {
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( DELTA_NULL_ACCOUNT ), [&]( account_object& a )
         {
            a.reward_delta_balance = ASSET( "1.000 TESTS" );
            a.reward_dbd_balance = ASSET( "1.000 DBD" );
            a.reward_vesting_balance = ASSET( "1.000000 VESTS" );
            a.reward_vesting_delta = ASSET( "1.000 TESTS" );
         });
         ...
         db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            ...
            gpo.current_dbd_supply += ASSET( "1.000 DBD" );
            gpo.pending_rewarded_vesting_shares += ASSET( "1.000000 VESTS" );
            gpo.pending_rewarded_vesting_delta += ASSET( "1.000 TESTS" );
         });
      });

      validate_database();

      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).dbd_balance == ASSET( "2.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).vesting_shares > ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).savings_balance == ASSET( "4.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).savings_dbd_balance == ASSET( "5.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_dbd_balance == ASSET( "1.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_delta_balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "1.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_vesting_delta == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "3.000 DBD" ) );

      BOOST_TEST_MESSAGE( "--- Generating block to clear balances" );
      generate_block();
      validate_database();

      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).vesting_shares == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).savings_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).savings_dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_delta_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).reward_vesting_delta == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "2.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "3.000 DBD" ) );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( generate_account_subsidies )
{
   try
   {
      ...
      const witness_schedule_object& wso = db->get_witness_schedule_object();
      BOOST_CHECK_EQUAL( wso.account_subsidy_rd.resource_unit, DELTA_ACCOUNT_SUBSIDY_PRECISION );
      ...
      while( true )
      {
         ...
         if( gpo.available_account_subsidies >= 100 * DELTA_ACCOUNT_SUBSIDY_PRECISION )
            break;
         generate_block();
         pool = pool + 5123 - ((249617279 * pool) >> DELTA_RD_DECAY_DENOM_SHIFT);
      }
...
BOOST_AUTO_TEST_CASE( account_subsidy_witness_limits )
{
   try
   {
      ...
      size_t n = size_t( db->get< witness_object, by_name >( db->get_scheduled_witness( 1 ) ).available_witness_account_subsidies / DELTA_ACCOUNT_SUBSIDY_PRECISION );
