...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      const account_object& init = db->get_account( DELTA_INIT_MINER_NAME );
      asset init_starting_balance = init.balance;

      account_create_operation op;

      op.new_account_name = "alice";
      op.creator = DELTA_INIT_MINER_NAME;
      ...
      BOOST_REQUIRE( acct.dbd_balance.amount.value == ASSET( "0.000 DBD" ).amount.value );
      ...
      tx.signatures.clear();
      tx.operations.clear();
      op.fee = asset( db->get_account( DELTA_INIT_MINER_NAME ).balance.amount + 1, DELTA_SYMBOL );
      op.new_account_name = "bob";
      tx.operations.push_back( op );
      sign( tx, init_account_priv_key );
      ...
      fund( DELTA_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
      vest( DELTA_INIT_MINER_NAME, DELTA_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

      BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
      op.creator = DELTA_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
   try
   {
      ...
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
      tx.signatures.clear();
      sign( tx, active_key );
      sign( tx, active_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
      ...
      tx.operations.push_back( op );
      sign( tx, active_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
   try
   {
      ...
      ACTORS( (alice)(bob) );
      generate_blocks( 60 / DELTA_BLOCK_INTERVAL );
      ...
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_posting_auth );
      ...
      sign( tx, bob_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

      BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
      tx.signatures.clear();
      sign( tx, bob_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( DELTA_CASHOUT_WINDOW_SECONDS ) ) );
      ...
      BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
      ...
      BOOST_REQUIRE( sam_comment.cashout_time == sam_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( sam_comment.root_comment == alice_comment.id );
      validate_database();

      generate_blocks( 60 * 5 / DELTA_BLOCK_INTERVAL + 1 );
      ...
      {
         o.total_reward_shares2 = delta::chain::util::evaluate_reward_curve( 10 );
      });
      ...
      BOOST_REQUIRE( mod_sam_comment.cashout_time == mod_sam_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
      validate_database();
      ...
      generate_blocks( 60 * 5 / DELTA_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
   try
   {
      ...
      vest( DELTA_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DELTA_MIN_TRANSACTION_EXPIRATION_LIMIT );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      generate_block();
      vote.weight = -1 * DELTA_100_PERCENT;
      tx.clear();
      tx.operations.push_back( vote );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      generate_blocks( DELTA_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / DELTA_BLOCK_INTERVAL );
      ...
      generate_blocks( DELTA_CASHOUT_WINDOW_SECONDS / DELTA_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * DELTA_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

         BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
         BOOST_REQUIRE( alice_comment.net_rshares.value == ( old_mana - alice.voting_manabar.current_mana ) - DELTA_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_manabar.current_mana ) - DELTA_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();

         BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

         generate_blocks( db->head_block_time() + DELTA_MIN_VOTE_INTERVAL_SEC );

         util::manabar old_manabar = db->get_account( "alice" ).voting_manabar;
         util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), DELTA_VOTING_MANA_REGENERATION_SECONDS );
         ...
         BOOST_REQUIRE( bob_comment.net_rshares.value == ( old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana ) - DELTA_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         generate_blocks( db->head_block_time() + fc::seconds( ( DELTA_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
         ...
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_manabar.current_mana ) - DELTA_VOTE_DUST_THRESHOLD );
         BOOST_REQUIRE( new_alice_comment.cashout_time == new_alice_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         op.weight = -1 * DELTA_100_PERCENT / 2;
         op.voter = "sam";
         op.author = "bob";
         op.permlink = "foo";
         tx.operations.clear();
         ...
         int64_t sam_weight = old_manabar.current_mana - db->get_account( "sam" ).voting_manabar.current_mana - DELTA_VOTE_DUST_THRESHOLD;
         ...
         BOOST_REQUIRE( new_bob_comment.cashout_time == new_bob_comment.created + DELTA_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         int64_t regenerated_power = (DELTA_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / DELTA_VOTING_MANA_REGENERATION_SECONDS;
         ...
         int64_t new_rshares = ( ( fc::uint128_t( db->get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / DELTA_100_PERCENT ).to_uint64() - DELTA_VOTE_DUST_THRESHOLD;

         BOOST_REQUIRE( db->get_comment( "alice", string( "foo" ) ).cashout_time == db->get_comment( "alice", string( "foo" ) ).created + DELTA_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( DELTA_1_PERCENT * 25 * ( get_voting_power( new_alice ) ) / DELTA_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "alice" ).voting_manabar.current_mana - DELTA_VOTE_DUST_THRESHOLD;
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( uint64_t( DELTA_1_PERCENT ) * 75 * uint64_t( alice_voting_power ) ) / DELTA_100_PERCENT;
         ...
         generate_blocks( fc::time_point_sec( ( new_bob_comment.cashout_time - DELTA_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + DELTA_BLOCK_INTERVAL ), true );
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - DELTA_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( DELTA_UPVOTE_LOCKOUT_SECONDS - DELTA_BLOCK_INTERVAL ) ) / DELTA_UPVOTE_LOCKOUT_SECONDS;
         ...
         new_rshares = old_manabar.current_mana - db->get_account( "dave" ).voting_manabar.current_mana - DELTA_VOTE_DUST_THRESHOLD;
         new_rshares = ( new_rshares * ( DELTA_UPVOTE_LOCKOUT_SECONDS - DELTA_BLOCK_INTERVAL - DELTA_BLOCK_INTERVAL ) ) / DELTA_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
   try
   {
      ...
      auto vests = asset( gpo.total_vesting_fund_delta.amount, DELTA_SYMBOL );
      ...
      BOOST_REQUIRE( gpo.total_vesting_fund_delta.amount.value == vests.amount.value );
      BOOST_REQUIRE( gpo.total_vesting_shares.amount.value == shares.amount.value );
      validate_database();

      op.to = "bob";
      op.amount = asset( 2000, DELTA_SYMBOL );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( DELTA_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( DELTA_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + DELTA_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         ...
         {
            gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.total_vesting_fund_delta;
            gpo.total_vesting_fund_delta = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
         });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
   try
   {
      ...
      op.fee = ASSET( "1.000 TESTS" );
      op.block_signing_key = signing_key.get_public_key();
      op.props.account_creation_fee = legacy_delta_asset::from_asset( asset(DELTA_MIN_ACCOUNT_CREATION_FEE + 10, DELTA_SYMBOL) );
      op.props.maximum_block_size = DELTA_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.proxy == DELTA_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxy == DELTA_PROXY_TO_SELF_ACCOUNT );
      ...
      BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.vesting_shares.amount );
      BOOST_REQUIRE( dave.proxy == DELTA_PROXY_TO_SELF_ACCOUNT );
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.proxy = DELTA_PROXY_TO_SELF_ACCOUNT;
      op.account = "bob";
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( alice.proxy == "sam" );
      BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( bob.proxy == DELTA_PROXY_TO_SELF_ACCOUNT );
      BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
      BOOST_REQUIRE( sam.proxy == "dave" );
      ...
      vote.account= "bob";
      vote.witness = DELTA_INIT_MINER_NAME;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( vote );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( db->get_witness( DELTA_INIT_MINER_NAME ).votes == ( alice.vesting_shares + bob.vesting_shares ).amount );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_witness( DELTA_INIT_MINER_NAME ).votes == bob.vesting_shares.amount );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( feed_publish_authorities )
{
   try
   {
      ...
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 DBD" ), ASSET( "1.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1.000 DBD" ), ASSET( "1000.000 TESTS" ) ); // 1000 DELTA : 1 DBD
      ...
      tx.operations.clear();
      tx.signatures.clear();
      op.exchange_rate = price( ASSET(" 1.000 DBD" ), ASSET( "1500.000 TESTS" ) );
      op.publisher = "alice";
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( new_bob.dbd_balance.amount.value == ASSET( "7.000 DBD" ).amount.value );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test failure when account does not have the required DBD" );
      op.owner = "alice";
      op.amount = ASSET( "5.000 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( new_alice.dbd_balance.amount.value == ASSET( "2.500 DBD" ).amount.value );
      validate_database();
      ...
      BOOST_REQUIRE( new_bob.balance.amount.value == ASSET( "3.000 TESTS" ).amount.value );
      BOOST_REQUIRE( new_bob.dbd_balance.amount.value == ASSET( "4.000 DBD" ).amount.value );
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + DELTA_CONVERSION_DELAY );
      ...
      BOOST_REQUIRE( convert_request->owner == op.owner );
      BOOST_REQUIRE( convert_request->requestid == op.requestid );
      BOOST_REQUIRE( convert_request->amount.amount.value == ASSET( "3.000 DBD" ).amount.value );
      //BOOST_REQUIRE( convert_request->premium == 100000 );
      BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + DELTA_CONVERSION_DELAY );
      validate_database();
   }
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
   ...
   try
   {
      ...
      fund( "dany", 5000 );
      DELTA_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      op.min_to_receive = ASSET( "10.000 DBD" );
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "1000.000 DBD" ).amount.value );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test failure when amount to receive is 0" );

      op.owner = "alice";
      op.min_to_receive = ASSET( "0.000 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "0.000 DBD" ).amount.value );
      validate_database();
      ...
      op.amount_to_sell = ASSET( "10.000 TESTS" );
      op.min_to_receive = ASSET( "15.000 DBD" );
      op.expiration = db->head_block_time() + fc::seconds( DELTA_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      ...
      // Fill 5 DELTA for 7.5 DBD

      op.owner = "bob";
      op.orderid = 1;
      op.amount_to_sell = ASSET( "7.500 DBD" );
      ...
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "10.000 TESTS" ), ASSET( "15.000 DBD" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( limit_order_idx.find( std::make_tuple( "bob", op.orderid ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "7.500 DBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "5.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "992.500 DBD" ).amount.value );
      ...
      BOOST_REQUIRE( fill_order_op.current_pays.amount.value == ASSET( "7.500 DBD" ).amount.value );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test filling an existing order fully, but the new order partially" );

      op.amount_to_sell = ASSET( "15.000 DBD" );
      ...
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "15.000 DBD" ), ASSET( "10.000 TESTS" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( limit_order_idx.find( std::make_tuple( "alice", 1 ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "15.000 DBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "10.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "977.500 DBD" ).amount.value );
      validate_database();
      ...
      op.min_to_receive = ASSET( "7.500 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "22.500 DBD" ).amount.value );
      ...
      op.orderid = 4;
      op.amount_to_sell = ASSET( "10.000 TESTS" );
      op.min_to_receive = ASSET( "11.000 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      op.owner = "bob";
      op.orderid = 4;
      op.amount_to_sell = ASSET( "12.000 DBD" );
      op.min_to_receive = ASSET( "10.000 TESTS" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "12.000 DBD" ), ASSET( "10.000 TESTS" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "975.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "33.500 DBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "25.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "965.500 DBD" ).amount.value );
      validate_database();
      ...
      op.orderid = 5;
      op.amount_to_sell = ASSET( "20.000 TESTS" );
      op.min_to_receive = ASSET( "22.000 DBD" );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "20.000 TESTS" ), ASSET( "22.000 DBD" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "955.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "45.500 DBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "35.909 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "954.500 DBD" ).amount.value );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( limit_order_create2_authorities )
{
   try
   {
      ...
      op.owner = "alice";
      op.amount_to_sell = ASSET( "1.000 TESTS" );
      op.exchange_rate = price( ASSET( "1.000 TESTS" ), ASSET( "1.000 DBD" ) );
      op.expiration = db->head_block_time() + fc::seconds( DELTA_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
   try
   {
      ...
      /// First check validation on price constructor level:
      {
        price broken_price;
        /// Invalid base value
        DELTA_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 DBD")),
          fc::exception);
        /// Invalid quote value
        DELTA_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 DBD")),
          fc::exception);
        /// Invalid symbol (same in base & quote)
        DELTA_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
          fc::exception);
      }
      ...
      */
      ...
      op.exchange_rate.quote = ASSET("1.000 DBD");
      ...
      BOOST_TEST_MESSAGE( "--- Test failure when expiration is too long" );
      op.amount_to_sell = ASSET( "10.000 TESTS" );
      op.exchange_rate = price( ASSET( "2.000 TESTS" ), ASSET( "3.000 DBD" ) );
      op.expiration = db->head_block_time() + fc::seconds( DELTA_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      op.orderid = 1;
      op.amount_to_sell = ASSET( "7.500 DBD" );
      op.exchange_rate = price( ASSET( "3.000 DBD" ), ASSET( "2.000 TESTS" ) );
      op.fill_or_kill = false;
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      ...
      BOOST_REQUIRE( limit_order->orderid == op.orderid );
      BOOST_REQUIRE( limit_order->for_sale == 5000 );
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "2.000 TESTS" ), ASSET( "3.000 DBD" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( limit_order_idx.find( std::make_tuple( "bob", op.orderid ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "7.500 DBD" ).amount.value );
      ...
      BOOST_REQUIRE( limit_order->orderid == 1 );
      BOOST_REQUIRE( limit_order->for_sale.value == 7500 );
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "3.000 DBD" ), ASSET( "2.000 TESTS" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( limit_order_idx.find( std::make_tuple( "alice", 1 ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "990.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "15.000 DBD" ).amount.value );
      ...
      op.orderid = 4;
      op.amount_to_sell = ASSET( "10.000 TESTS" );
      op.exchange_rate = price( ASSET( "1.000 TESTS" ), ASSET( "1.100 DBD" ) );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      op.orderid = 4;
      op.amount_to_sell = ASSET( "12.000 DBD" );
      op.exchange_rate = price( ASSET( "1.200 DBD" ), ASSET( "1.000 TESTS" ) );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, bob_private_key );
      ...
      BOOST_REQUIRE( limit_order->orderid == 5 );
      BOOST_REQUIRE( limit_order->for_sale.value == 9091 );
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "1.000 TESTS" ), ASSET( "1.100 DBD" ) ) );
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( DBD_SYMBOL, DELTA_SYMBOL ) );
      BOOST_REQUIRE( alice.balance.amount.value == ASSET( "955.000 TESTS" ).amount.value );
      BOOST_REQUIRE( alice.dbd_balance.amount.value == ASSET( "45.500 DBD" ).amount.value );
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "35.909 TESTS" ).amount.value );
      ...
      op.orderid = 1;
      op.amount_to_sell = ASSET( "20.000 TESTS" );
      op.exchange_rate = price( ASSET( "1.000 TESTS" ), ASSET( "0.500 DBD" ) );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      ...
      op.orderid = 6;
      op.amount_to_sell = ASSET( "20.000 TESTS" );
      op.exchange_rate = price( ASSET( "1.000 TESTS" ), ASSET( "2.000 DBD" ) );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      op.owner = "dave";
      op.orderid = 1;
      op.amount_to_sell = ASSET( "25.000 DBD" );
      op.exchange_rate = price( ASSET( "1.000 DBD" ), ASSET( "0.010 TESTS" ) );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, dave_private_key );
      ...
      BOOST_REQUIRE( fill_order_op.current_orderid == 1 );
      BOOST_REQUIRE( fill_order_op.current_pays == ASSET( "10.000 DBD" ) );
      ...
      BOOST_REQUIRE( fill_order_op.current_owner == "dave" );
      BOOST_REQUIRE( fill_order_op.current_orderid == 1 );
      BOOST_REQUIRE( fill_order_op.current_pays == ASSET( "15.000 DBD" ) );
      ...
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "1.000 TESTS" ), ASSET( "1.000 DBD" ) ) );

      limit_order = limit_order_idx.find( std::make_tuple( "alice", 6 ) );
      BOOST_REQUIRE( limit_order->seller == "alice" );
      BOOST_REQUIRE( limit_order->orderid == 6 );
      BOOST_REQUIRE( limit_order->for_sale.value == 20000 );
      BOOST_REQUIRE( limit_order->sell_price == price( ASSET( "1.000 TESTS" ), ASSET( "2.000 DBD" ) ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_authorities )
{
   try
   {
      ...
      c.min_to_receive = ASSET( "1.000 DBD" );
      c.expiration = db->head_block_time() + fc::seconds( DELTA_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_apply )
{
   try
   {
      ...
      create.min_to_receive = ASSET( "7.500 DBD" );
      create.expiration = db->head_block_time() + fc::seconds( DELTA_MAX_LIMIT_ORDER_EXPIRATION );
      tx.operations.clear();
      tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      vest( DELTA_INIT_MINER_NAME, "bob", asset( 1000, DELTA_SYMBOL ) );
      ...
      generate_blocks( db->head_block_time() + DELTA_OWNER_UPDATE_LIMIT );
      ...
      generate_blocks( db->head_block_time() + DELTA_OWNER_UPDATE_LIMIT + fc::seconds( DELTA_BLOCK_INTERVAL ) );
      ...
      BOOST_REQUIRE( req_itr->expires == db->head_block_time() + DELTA_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
      auto expires = req_itr->expires;
      ++req_itr;
      BOOST_REQUIRE( req_itr == request_idx.end() );

      generate_blocks( time_point_sec( expires - DELTA_BLOCK_INTERVAL ), true );
      ...
      generate_blocks( db->head_block_time() + ( DELTA_OWNER_AUTH_RECOVERY_PERIOD - DELTA_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      DELTA_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
      DELTA_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
      DELTA_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
      ...
      generate_blocks( db->head_block_time() + DELTA_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( DELTA_BLOCK_INTERVAL ), true );
      // cannot request account recovery until recovery account is approved
      DELTA_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
      generate_blocks(1);
      // cannot finish account recovery until requested
      DELTA_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
      // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.from = "alice";
      op.to = "bob";
      op.dbd_amount = ASSET( "1.000 DBD" );
      op.delta_amount = ASSET( "1.000 TESTS" );
      op.escrow_id = 0;
      op.agent = "sam";
      op.fee = ASSET( "0.100 TESTS" );
      op.json_meta = "";
      ...
      op.dbd_amount.symbol = DELTA_SYMBOL;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when delta symbol != DELTA" );
      op.dbd_amount.symbol = DBD_SYMBOL;
      op.delta_amount.symbol = DBD_SYMBOL;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when fee symbol != DBD and fee symbol != DELTA" );
      op.delta_amount.symbol = DELTA_SYMBOL;
      op.fee.symbol = VESTS_SYMBOL;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when dbd == 0 and delta == 0" );
      op.fee.symbol = DELTA_SYMBOL;
      op.dbd_amount.amount = 0;
      op.delta_amount.amount = 0;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when dbd < 0" );
      op.dbd_amount.amount = -100;
      op.delta_amount.amount = 1000;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when delta < 0" );
      op.dbd_amount.amount = 1000;
      op.delta_amount.amount = -100;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
   try
   {
      ...
      auto alice_delta_balance = alice.balance - op.delta_amount - op.fee;
      auto alice_dbd_balance = alice.dbd_balance - op.dbd_amount;
      auto bob_delta_balance = bob.balance;
      auto bob_dbd_balance = bob.dbd_balance;
      auto sam_delta_balance = sam.balance;
      ...
      BOOST_REQUIRE( escrow.delta_balance == op.delta_amount );
      ...
      BOOST_REQUIRE( alice.balance == alice_delta_balance );
      BOOST_REQUIRE( alice.dbd_balance == alice_dbd_balance );
      BOOST_REQUIRE( bob.balance == bob_delta_balance );
      BOOST_REQUIRE( bob.dbd_balance == bob_dbd_balance );
      BOOST_REQUIRE( sam.balance == sam_delta_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
   try
   {
      ...
      et_op.to = "bob";
      et_op.agent = "sam";
      et_op.delta_amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( escrow.dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( escrow.delta_balance == ASSET( "1.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DELTA_BLOCK_INTERVAL );
      sign( tx, bob_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      ...
      DELTA_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
      BOOST_REQUIRE( alice.balance == ASSET( "10.000 TESTS" ) );
      validate_database();
      ...
      generate_blocks( et_op.ratification_deadline + DELTA_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
   try
   {
      ...
      et_op.delta_amount = ASSET( "1.000 TESTS" );
      et_op.fee = ASSET( "0.100 TESTS" );
      et_op.ratification_deadline = db->head_block_time() + DELTA_BLOCK_INTERVAL;
      et_op.escrow_expiration = db->head_block_time() + 2 * DELTA_BLOCK_INTERVAL;
      ...
      BOOST_REQUIRE( escrow.delta_balance == et_op.delta_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when delta == 0 and dbd == 0" );
      op.dbd_amount.amount = 0;
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when delta is not delta symbol" );
      op.dbd_amount.symbol = DBD_SYMBOL;
      op.delta_amount = ASSET( "1.000 DBD" );
      DELTA_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
   try
   {
      ...
      op.delta_amount = ASSET( "0.100 TESTS" );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).delta_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).delta_balance == ASSET( "0.800 TESTS" ) );
      ...
      BOOST_TEST_MESSAGE( "--- failure when releasing less delta than available" );
      op.delta_amount = ASSET( "0.000 TESTS" );
      op.dbd_amount = ASSET( "1.000 DBD" );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).delta_balance == ASSET( "0.700 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).delta_balance == ASSET( "0.600 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).delta_balance == ASSET( "0.500 TESTS" ) );


      BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
      tx.clear();
      op.delta_amount = ASSET( "0.500 TESTS" );
      tx.operations.push_back( op );
      sign( tx, sam_private_key );
      db->push_transaction( tx, 0 );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "9.700 TESTS" ) );
      DELTA_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).delta_balance == ASSET( "0.900 TESTS" ) );
      ...
      BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).delta_balance == ASSET( "0.800 TESTS" ) );
      ...
      tx.clear();
      op.delta_amount = ASSET( "0.600 TESTS" );
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      op.amount = ASSET( "1.000 DBD" );
      op.validate();


      BOOST_TEST_MESSAGE( "success when amount is DELTA" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "10.000 DBD" ) );
      ...
      BOOST_TEST_MESSAGE( "--- success transferring DELTA to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "9.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "1.000 DBD" ) );
      validate_database();


      BOOST_TEST_MESSAGE( "--- success transferring DELTA to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "8.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).savings_dbd_balance == ASSET( "1.000 DBD" ) );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing DELTA to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + DELTA_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "9.000 DBD" ) );
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing DELTA to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_dbd_balance == ASSET( "8.000 DBD" ) );
      ...
      generate_blocks( db->head_block_time() + DELTA_SAVINGS_WITHDRAW_TIME - fc::seconds( DELTA_BLOCK_INTERVAL ), true );

      BOOST_REQUIRE( db->get_account( "alice" ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == 4 );
      validate_database();
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == ASSET( "1.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).balance == ASSET( "1.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "bob" ).dbd_balance == ASSET( "1.000 DBD" ) );
      ...
      for( int i = 0; i < DELTA_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = DELTA_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == DELTA_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( itr->effective_date == db->head_block_time() + DELTA_OWNER_AUTH_RECOVERY_PERIOD );
      ...
      vote.weight = DELTA_1_PERCENT * 50;
      tx.clear();
      tx.operations.push_back( vote );
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.account = "alice";
      op.reward_delta = ASSET( "0.000 TESTS" );
      op.reward_dbd = ASSET( "0.000 DBD" );
      ...
      op.reward_delta.amount = 1000;
      op.validate();

      op.reward_delta.amount = 0;
      ...
      BOOST_TEST_MESSAGE( "Testing wrong DELTA symbol" );
      op.reward_delta = ASSET( "1.000 DBD" );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong DBD symbol" );
      op.reward_delta = ASSET( "1.000 TESTS" );
      op.reward_dbd = ASSET( "1.000 TESTS" );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );


      BOOST_TEST_MESSAGE( "Testing wrong VESTS symbol" );
      op.reward_dbd = ASSET( "1.000 DBD" );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_apply )
{
   try
   {
      ...
      db_plugin->debug_update( []( database& db )
      {
         ...
         {
            a.reward_delta_balance = ASSET( "10.000 TESTS" );
            a.reward_dbd_balance = ASSET( "10.000 DBD" );
            a.reward_vesting_balance = ASSET( "10.000000 VESTS" );
            a.reward_vesting_delta = ASSET( "10.000 TESTS" );
         });

         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
         {
            gpo.current_supply += ASSET( "20.000 TESTS" );
            gpo.current_dbd_supply += ASSET( "10.000 DBD" );
            gpo.virtual_supply += ASSET( "20.000 TESTS" );
            gpo.pending_rewarded_vesting_shares += ASSET( "10.000000 VESTS" );
            gpo.pending_rewarded_vesting_delta += ASSET( "10.000 TESTS" );
         });
      });
      ...
      auto alice_delta = db->get_account( "alice" ).balance;
      auto alice_dbd = db->get_account( "alice" ).dbd_balance;
      auto alice_vests = db->get_account( "alice" ).vesting_shares;


      BOOST_TEST_MESSAGE( "--- Attempting to claim more DELTA than exists in the reward balance." );

      claim_reward_balance_operation op;
      signed_transaction tx;

      op.account = "alice";
      op.reward_delta = ASSET( "20.000 TESTS" );
      op.reward_dbd = ASSET( "0.000 DBD" );
      op.reward_vests = ASSET( "0.000000 VESTS" );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_delta + op.reward_delta );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_delta_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == alice_dbd + op.reward_dbd );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dbd_balance == ASSET( "10.000 DBD" ) );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_delta == ASSET( "5.000 TESTS" ) );
      validate_database();
      ...
      op.reward_delta = ASSET( "10.000 TESTS" );
      op.reward_dbd = ASSET( "10.000 DBD" );
      tx.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      db->push_transaction( tx, 0 );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_delta + op.reward_delta );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_delta_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == alice_dbd + op.reward_dbd );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dbd_balance == ASSET( "0.000 DBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).vesting_shares == alice_vests + op.reward_vests );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_delta == ASSET( "0.000 TESTS" ) );
            validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
   try
   {
      ...
      signed_transaction tx;
      ACTORS( (alice)(bob) )
      vest( DELTA_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      util::manabar_params params( util::get_effective_vesting_shares( db->get_account( "bob" ) ), DELTA_VOTING_MANA_REGENERATION_SECONDS );
      old_manabar.regenerate_mana( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - DELTA_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr->rshares == old_manabar.current_mana - db->get_account( "bob" ).voting_manabar.current_mana - DELTA_VOTE_DUST_THRESHOLD );

      generate_block();
      ACTORS( (sam)(dave) )
      generate_block();

      vest( DELTA_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
      ...
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, sam_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      BOOST_REQUIRE( gpo.delegation_return_period == DELTA_DELEGATION_RETURN_PERIOD_HF20 );
      ...
      generate_blocks( exp_obj->expiration + DELTA_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DELTA_100_PERCENT + 1 ) );
      op.extensions.insert( b );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DELTA_1_PERCENT * 75 ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), DELTA_1_PERCENT * 75 ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DELTA_1_PERCENT * 2 ) );
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DELTA_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( "bob", DELTA_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( "alice", DELTA_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      DELTA_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), DELTA_1_PERCENT ) );

      for( size_t i = 0; i < 8; i++ )
      {
         b.beneficiaries.push_back( beneficiary_route_type( account_name_type( DELTA_INIT_MINER_NAME + fc::to_string( i ) ), DELTA_1_PERCENT ) );
      }
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


      BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), DELTA_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      tx.clear();
      tx.operations.push_back( op );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * DELTA_1_PERCENT ) );
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * DELTA_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      ...
      b.beneficiaries.clear();
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * DELTA_1_PERCENT ) );
      op.extensions.clear();
      op.extensions.insert( b );
      sign( tx, alice_private_key );
      DELTA_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
      ...
      generate_blocks( db->get_comment( "alice", string( "test" ) ).cashout_time - DELTA_BLOCK_INTERVAL );

      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
         {
            gpo.current_supply -= gpo.total_reward_fund_delta;
            gpo.total_reward_fund_delta = ASSET( "100.000 TESTS" );
            gpo.current_supply += gpo.total_reward_fund_delta;
         });
      });
      ...
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_delta.amount + db->get_account( "bob" ).reward_dbd_balance.amount + db->get_account( "sam" ).reward_vesting_delta.amount + db->get_account( "sam" ).reward_dbd_balance.amount == db->get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_dbd_balance.amount + db->get_account( "alice" ).reward_vesting_delta.amount ) == db->get_account( "bob" ).reward_vesting_delta.amount + db->get_account( "bob" ).reward_dbd_balance.amount + 2 );
      BOOST_REQUIRE( ( db->get_account( "alice" ).reward_dbd_balance.amount + db->get_account( "alice" ).reward_vesting_delta.amount ) * 2 == db->get_account( "sam" ).reward_vesting_delta.amount + db->get_account( "sam" ).reward_dbd_balance.amount + 3 );
      BOOST_REQUIRE( db->get_account( "bob" ).reward_vesting_delta.amount == db->get_account( "bob" ).reward_dbd_balance.amount );
      BOOST_REQUIRE( db->get_account( "sam" ).reward_vesting_delta.amount == db->get_account( "sam" ).reward_dbd_balance.amount + 1 );
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
   try
   {
      ...
      prop_op.owner = "alice";
      DELTA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      prop_op.props[ "account_creation_fee" ] = fc::raw::pack_to_vector( ASSET( "2.000 DBD" ) );
      DELTA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below DELTA_MIN_BLOCK_SIZE_LIMIT" );
      prop_op.props.erase( "account_creation_fee" );
      prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( DELTA_MIN_BLOCK_SIZE_LIMIT - 1 );
      DELTA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when setting dbd_interest_rate to DELTA_100_PERCENT + 1" );
      prop_op.props[ "dbd_interest_rate" ].clear();
      prop_op.props[ "dbd_interest_rate" ] = fc::raw::pack_to_vector( DELTA_100_PERCENT + 1 );
      DELTA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( "--- failure when setting new dbd_exchange_rate with DBD / DELTA" );
      prop_op.props.erase( "dbd_interest_rate" );
      prop_op.props[ "dbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET( "1.000 TESTS" ), ASSET( "10.000 DBD" ) ) );
      DELTA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
      ...
      uint64_t unit = uint64_t(1) << DELTA_RD_DECAY_DENOM_SHIFT;

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / DELTA_BLOCKS_PER_YEAR ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / DELTA_BLOCKS_PER_DAY ) );
      prop_op.validate();

      BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
      prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/DELTA_BLOCK_INTERVAL) ) );
      prop_op.validate();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
   try
   {
      ...
      expected_keys.push_back( authority( 1, DELTA_NULL_ACCOUNT, 1 ) );
      op.get_required_authorities( key_auths );
      BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
   try
   {
      ...
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.maximum_block_size == DELTA_MIN_BLOCK_SIZE_LIMIT + 1 );
      ...
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "dbd_exchange_rate" ] = fc::raw::pack_to_vector( price( ASSET(" 1.000 DBD" ), ASSET( "100.000 TESTS" ) ) );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.dbd_exchange_rate == price( ASSET( "1.000 DBD" ), ASSET( "100.000 TESTS" ) ) );
      BOOST_REQUIRE( alice_witness.last_dbd_exchange_update == db->head_block_time() );
      ...
      prop_op.props[ "key" ].clear();
      prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
      prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( DELTA_ACCOUNT_SUBSIDY_PRECISION );
      tx.clear();
      tx.operations.push_back( prop_op );
      sign( tx, signing_key );
      db->push_transaction( tx, 0 );
      BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == DELTA_ACCOUNT_SUBSIDY_PRECISION );

      BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
      uint64_t day_decay = ( uint64_t(1) << DELTA_RD_DECAY_DENOM_SHIFT ) / DELTA_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
   try
   {
      ...
      generate_blocks( DELTA_BLOCKS_PER_HOUR/2 );
      ...
      BOOST_REQUIRE( db->get_account( DELTA_NULL_ACCOUNT ).balance == ASSET( "5.000 TESTS" ) );
      ...
      BOOST_CHECK_EQUAL( c_subs, prev_c_subs - DELTA_ACCOUNT_SUBSIDY_PRECISION );
      BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
      ...
      int64_t new_value = prev_c_subs - DELTA_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
      new_value = new_value
          + 5000                                                             // Budget
          - ((new_value*249617279) >> DELTA_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( bob.balance.amount.value == ASSET( "0.000 TESTS" ).amount.value );
      BOOST_REQUIRE( bob.dbd_balance.amount.value == ASSET( "0.000 DBD" ).amount.value );
      ...
      db_plugin->debug_update( [=]( database& db )
      {
         db.modify( db.get_account( DELTA_TEMP_ACCOUNT ), [&]( account_object& a )
         {
