...
function print_help_and_quit {
   echo "Usage: path_to_tested_deltad path_to_reference_deltad path_to_test_blockchain_directory"
   echo "       path_to_reference_blockchain_directory number_of_blocks_to_replay [number_of_jobs [--dont-copy-config]]"
   echo "Example: ~/work/deltait/delta/build/Release/programs/deltad/deltad ~/master/deltait/delta/build/Release/programs/deltad/deltad ~/deltait/delta/work1 ~/deltait/delta/work2 5000000"
...
function run_test_group {
   ...
   echo Running ./$GROUP_TEST_SCRIPT $JOBS $TEST_DELTAD_PATH $REF_DELTAD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   ./$GROUP_TEST_SCRIPT $JOBS $TEST_DELTAD_PATH $REF_DELTAD_PATH $TEST_WORK_PATH $REF_WORK_PATH $BLOCK_LIMIT $COPY_CONFIG
   EXIT_CODE=$?
   if [ $EXIT_CODE -ne 0 ]
   then
      EXIT_CODE=-1
...
check_deltad_path $TEST_DELTAD_PATH
check_deltad_path $REF_DELTAD_PATH
