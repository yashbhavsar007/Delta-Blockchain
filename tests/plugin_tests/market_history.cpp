...
BOOST_AUTO_TEST_CASE( mh_test )
{
   ...
   try
   {
      ...
      op.owner = "alice";
      op.amount_to_sell = ASSET( "1.000 DBD" );
      ...
      op.min_to_receive = ASSET( "0.750 DBD" );
      tx.operations.push_back( op );
      ...
      op.min_to_receive = ASSET( "0.500 DBD" );
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      ...
      op.owner = "alice";
      op.amount_to_sell = ASSET( "0.500 DBD" );
      ...
      op.min_to_receive = ASSET( "0.250 DBD" );
      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DELTA_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, bob_private_key );
      db->push_transaction( tx, 0 );
      ...
      BOOST_REQUIRE( bucket->non_delta.high == ASSET( "0.750 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.low == ASSET( "0.750 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.open == ASSET( "0.750 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.close == ASSET( "0.750 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.volume == ASSET( "0.750 DBD" ).amount );
      ...
      BOOST_REQUIRE( bucket->non_delta.high == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.low == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.open == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.close == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.volume == ASSET( "0.250 DBD" ).amount );
      ...
      BOOST_REQUIRE( bucket->non_delta.open == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.close == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.volume == ASSET( "0.500 DBD" ).amount );
      bucket++;
      ...
      BOOST_REQUIRE( bucket->non_delta.open == ASSET( "0.750 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.close == ASSET( "0.250 DBD" ).amount );
      BOOST_REQUIRE( bucket->non_delta.volume == ASSET( "1.500 DBD" ).amount );
      bucket++;
      ...
      BOOST_REQUIRE( order->op.open_pays == ASSET( "0.750 DBD" ) );
      order++;
      ...
      BOOST_REQUIRE( order->op.open_owner == "alice" );
      BOOST_REQUIRE( order->op.open_orderid == 0 );
      BOOST_REQUIRE( order->op.open_pays == ASSET( "0.250 DBD" ) );
      order++;

      BOOST_REQUIRE( order->time == fill_order_c_time );
      BOOST_REQUIRE( order->op.current_owner == "alice" );
      BOOST_REQUIRE( order->op.current_orderid == 0 );
      BOOST_REQUIRE( order->op.current_pays == ASSET( "0.250 DBD" ) );
