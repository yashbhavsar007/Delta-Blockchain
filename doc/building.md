...
Builds deltad to be a consensus-only low memory node. Data and fields not
...
### BUILD_DELTA_TESTNET=[OFF/ON]

Builds delta for use in a private testnet. Also required for building unit tests.
...
For Ubuntu 16.04 users, after installing the right packages with `apt` Delta
...
Delta requires Boost 1.58 (as in Ubuntu 16.04) and works with versions up to 1.60 (including).
So building Delta on Ubuntu 14.04 requires downloading and installing a more recent
version of Boost.
...
Ubuntu 14.04) when compiling in C++11 mode (which Delta does).
...
Then the instructions are the same as for delta:
...
delta. Until then, this will allow you to install boost 1.60.0.
...
e.g.:
    ...
    make -j$(sysctl -n hw.logicalcpu) deltad

This will only build `deltad`.
