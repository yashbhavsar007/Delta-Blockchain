...
One way to think about decay is "resources expire."  Delta had relatively little activity over its
...
- The maximum RC an account can have is equal to its DeltaX (technically, VESTS).
- It takes 5 days for RC to regenerate from 0% to 100%.  (So more TX means more RC's per hour.)
...
of the day), and weekly variations (people use Delta more/less on weekends/holidays).  The default decay rate (347321) corresponds to a half-life of
...
100% of the DeltaX stake is constantly using all of its RC to do transactions that only require a single resource, the usage will be
...
less, than the sum of each individual user's decisions.  The budget creates market forces that influence these decisions:  You need X DeltaX
...
depends very much on the sum of users' individual decisions about how many resources to consume and how much TX heavy consumers are willing
...
- A value of 10,000 (`DELTA_ACCOUNT_SUBSIDY_PRECISION`) represents a budget of one subsidized account per block.
- `account_subsidy_decay` is the per-block decay rate.  A value of `2^36` (`DELTA_RD_DECAY_DENOM_SHIFT`) represents 100% decay rate.
...
DELTA_BLOCKS_PER_DAY = 20*60*24
DELTA_RD_DECAY_DENOM_SHIFT = 36
DELTA_BLOCKS_PER_DAY = 20*60*24
DELTA_MAX_WITNESSES = 21
DELTA_MAX_VOTED_WITNESSES_HF17 = 20

f = lambda d : int(0.5 + (1 << DELTA_RD_DECAY_DENOM_SHIFT) * (-math.expm1(-math.log(2.0) / (DELTA_BLOCKS_PER_DAY * d * DELTA_MAX_VOTED_WITNESSES_HF17 / DELTA_MAX_WITNESSES))))
...
- How do you balance these concerns against allowing users to transact regularly, with little TX?
- How much TX do legitimate users and service providers delegate, relative to how much TX spammers have?

Generous resource budgets mean users can transact a lot, and don't need a lot of TX.  This is great for
...
- Upgrading consensus limits is a hardfork, all `deltad` nodes must upgrade.
...
- If a witness ignores the subsidized account limit, people will get new accounts for free, that normally cost DELTA to create.  This is a medium-sized economic problem.
...
For resource limits, having witness-only upgrades outweighs the problem.  Witnesses who have been around a long time know that, in the ancient past, Delta's bandwidth
...
we should never implement such operations in the Delta source code!  Even the worst-case CPU cycles / memory bytes consumed by an attacker spamming the most
