This is developer documentation for creating brand-new operations on the DELTA blockchain.
...
- (4) Add operation to `delta::protocol::operation`
...
- (4a) The file `operations.hpp` defines the `delta::protocol::operation`
...
- (5a) You must add `DELTA_DEFINE_EVALUATOR` macro in `evaluator.hpp` to
...
- (9a) Add `smt_token_object_type` to `enum object_type` in `delta_objects.hpp` and add
to `FC_REFLECT_ENUM` bubble list at the bottom of that file
- (9b) Declare (but do not define) `class smt_token_object;` in `delta_objects.hpp`
- (9c) Define `typedef oid< smt_token_object > smt_token_id_type` in `delta_objects.hpp`
...
- (9e) All SMT objects are consensus, and therefore should exist in `delta::chain` namespace
...
definition.
- (9n) Macro
`CHAINBASE_SET_INDEX_TYPE( delta::chain::smt_token_object, delta::chain::smt_token_index )`
...
defined by an `enum` in `delta_object_types.hpp`, any new objects must be added here.  In SQL
...
alias `typedef oid< smt_token_object > smt_id_type` is added to `delta_object_types.hpp`.
...
is designed as a reusable library, not tightly coupled to Delta, it contains no reference
to any `delta` namespaces.  So the name `by_id` must refer to `chainbase::by_id`.  If you
define a `struct by_id;` in the `delta::chain` namespace, the result will be that every
...
- (9m) All indexes used in Delta must be `ordered_unique`.  In theory, hashed or non-unique
