...
The role of a witness in the Delta Blockchain is verify incoming transactions, produce blocks when scheduled, and partake in the Delta governance model by voting on several parameters.

These parameters control various aspects of the operation of the blockchain that are not easily defined in code at compile time. One example is the DELTA price feed that defines the conversion rate between DELTA and DBD.
...
This operation was added in Delta v0.20.0 to replace the `witness_update_operation` which was not easily extendable. While it is recommended to use `witness_set_properties_operation`, `witness_update_operation` will continue to work.

## Properties

### account_creation_fee

This is the fee in DELTA that must be paid to create an account. This field must be non-negative.
...
The exchange rate for DELTA/SDB to be used for printing DBD as rewards as well as DBD->DELTA conversions.
